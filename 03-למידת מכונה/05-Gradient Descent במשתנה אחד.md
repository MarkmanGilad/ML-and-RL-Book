...
### האלגוריתם — Stochastic Gradient Descent (SGD)
...
<figure>
<img src="../assets/slides/ac6c682411/image4.png" alt="עקומת ההפסד כתלות במשקל: מנקודת ההתחלה יורדים בצעדים קטנים לאורך העקומה, בכיוון ההפוך לשיפוע, עד לנקודת המינימום." style="max-width:100%;height:auto;">
<figcaption>הציר האופקי הוא המשקל (Weight) והאנכי הוא ההפסד (Cost). מנקודת ההתחלה (Initial Weight) יורדים בצעדים קטנים (Incremental Step) לאורך העקומה, בכיוון ההפוך לשיפוע המשיק (Gradient), עד לנקודת המינימום (Minimum Cost).</figcaption>
</figure>

<!-- editorlm-source-ref: [sources/ML/pdf/4. Gradient Descent.pdf#L18-L38] -->

**מדוע לא פשוט לגזור ולהשוות לאפס?** בשיעורי המתמטיקה למדנו דרך מדויקת למצוא מינימום: גוזרים את הפונקציה, משווים את הנגזרת לאפס ופותרים את המשוואה. לפרבולה שנפגוש מיד הדרך הזו עובדת מצוין: הנגזרת של <span dir="ltr">w² − 4w + 8</span> היא <span dir="ltr">2w − 4</span>, היא מתאפסת כאשר w=2, וזהו המינימום. אלא שהדרך הזו דורשת שני דברים: לכתוב את פונקציית הנגזרת בצורה מפורשת, ולפתור את המשוואה "נגזרת שווה אפס".

ברשת נוירונים פונקציית ההפסד היא הרכבה של אלפי פעולות על מיליוני משקלים. אין דרך אלגברית לכתוב את המשוואה "הנגזרת שווה אפס" עבור כל המשקלים יחד ולפתור אותה. מה שכן אפשר לעשות, בכל נקודה נתונה, הוא לחשב את *הערך* של הנגזרת באותה נקודה, וזה בדיוק מה ש־Autograd עושה בשבילנו. Gradient Descent בנוי על היכולת הזו בלבד: איננו פותרים משוואה, אלא חוזרים שוב ושוב על צעד פשוט, מחשבים את הנגזרת בנקודה הנוכחית ומתקדמים מעט במורד. איננו צריכים לדעת דבר על צורת הפונקציה כולה. לכן זו השיטה שבה מאמנים רשתות נוירונים, ולכן נלמד אותה כאן דווקא על פונקציות פשוטות, שבהן עדיין אפשר לבדוק את התשובה ביד.

<!-- editorlm-source-ref: [sources/ML/pdf/4. Gradient Descent.pdf#L30-L38] -->

נתרגם עתה את הרעיון ל**אלגוריתם**: רצף פעולות קבוע שחוזרים עליו שוב ושוב. שמו המלא של האלגוריתם הוא **SGD — Stochastic Gradient Descent**, ירידה בגרדיאנט סטוכסטית. סטוכסטי פירושו אקראי: האלגוריתם אינו יודע היכן המינימום, ולכן הוא מתחיל מנקודה אקראית. באימון רשתות נוירונים מצטרף לכך מקור אקראיות נוסף, שנכיר בהמשך: בכל צעד מחשבים את הנגזרת על קבוצה אקראית של דוגמאות מתוך הנתונים.

נתונה פונקציה <span dir="ltr">f(w)</span>, למשל פונקציית ההפסד, ואנו מחפשים את הערך w שבו היא מינימלית. אלו שלבי האלגוריתם:

1. **בוחרים נקודת התחלה אקראית** w, וקובעים את **קצב הלמידה — learning rate**: מספר קטן שקובע עד כמה גדול יהיה כל צעד.
2. **חישוב קדימה:** מחשבים את ערך הפונקציה בנקודה הנוכחית, <span dir="ltr">f(w)</span>.
3. **חישוב לאחור:** מחשבים את הנגזרת של הפונקציה בנקודה זו.
4. **עדכון:** מזיזים את w צעד קטן בכיוון ההפוך לנגזרת.
5. **מאפסים את הנגזרת** וחוזרים לשלב 2.

כלל העדכון בשלב 4 הוא:
...
לדוגמה, אם הנגזרת היא 2 וקצב הלמידה הוא 0.01, מקטינים את המשקל ב־0.02. סימן המינוס הוא שמבטיח שהצעד יהיה תמיד במורד. שימו לב שגודל הצעד תלוי גם בנגזרת: רחוק מהמינימום, במקום שבו השיפוע תלול, הצעדים גדולים; ככל שמתקרבים למינימום השיפוע מתמתן והצעדים נעשים קטנים מאליהם. האיפוס בשלב 5 נדרש מפני ש־Autograd צובר נגזרות, כפי שראינו בפרק הקודם, ואנו רוצים שכל איטרציה תתחיל מנגזרת נקייה.

<figure>
<img src="../assets/slides/ac6c682411/image5.jpeg" alt="עקומת הפסד בצורת U: מנקודת התחלה אקראית בצד שמאל יורדות נקודות בצעדים הולכים וקטנים עד לנקודת המינימום." style="max-width:100%;height:auto;">
<figcaption>מנקודת התחלה אקראית (Random initial value) האלגוריתם מתקדם צעד אחר צעד (Learning step) לאורך עקומת ההפסד עד לנקודת המינימום. הצעדים מתקצרים ככל שהשיפוע מתמתן.</figcaption>
</figure>

**מתי עוצרים?** להלכה, האלגוריתם הגיע ליעדו כשהנגזרת קרובה לאפס: הקרקע שטוחה ואין עוד לאן לרדת. למעשה, בלמידת מכונה נהוג לקבוע מראש כמה איטרציות יבצע האימון, ובסיומו להסתכל על גרף ההפסד לאורך האיטרציות. אם הגרף התיישר, הגענו למינימום; אם הוא עדיין יורד, מריצים איטרציות נוספות. כך גם בקוד שבפרק זה: הלולאה רצה מספר קבוע של פעמים, ואנו בודקים בפלט אם המשקל והנגזרת התייצבו.
...
<figure>
<img src="../assets/gradient-descent/loss-parabola.png" alt="פרבולה אדומה של loss = w² − 4w + 8 בתחום 0 עד 4, עם תחתית ב־w=2." style="max-width:100%;height:auto;">
<figcaption>גרף הפונקציה: הפרבולה יורדת עד w=2 ועולה בחזרה.</figcaption>
</figure>

בפלט רואים שערכי ההפסד יורדים עד 4 ואז עולים בחזרה. הערך הנמוך ביותר, 4, מתקבל באמצע הרשימה — עבור w=2, וזו גם תחתית הפרבולה שבגרף. זהו המינימום שאנו מצפים שהאלגוריתם ימצא.
...
```text
epoch= 0 W= 5.50000 model=16.25000 grad= 7.00000
epoch= 1 W= 4.80000 model=11.84000 grad= 5.60000
epoch= 2 W= 4.24000 model=9.01760 grad= 4.48000
epoch= 3 W= 3.79200 model=7.21126 grad= 3.58400
epoch= 4 W= 3.43360 model=6.05521 grad= 2.86720
epoch= 5 W= 3.14688 model=5.31533 grad= 2.29376
epoch= 6 W= 2.91750 model=4.84181 grad= 1.83501
epoch= 7 W= 2.73400 model=4.53876 grad= 1.46801
epoch= 8 W= 2.58720 model=4.34481 grad= 1.17441
epoch= 9 W= 2.46976 model=4.22068 grad= 0.93952
epoch= 10 W= 2.37581 model=4.14123 grad= 0.75162
epoch= 20 W= 2.04035 model=4.00163 grad= 0.08070
epoch= 30 W= 2.00433 model=4.00002 grad= 0.00867
epoch= 40 W= 2.00046 model=4.00000 grad= 0.00093
epoch= 50 W= 2.00005 model=4.00000 grad= 0.00010
epoch= 60 W= 2.00001 model=4.00000 grad= 0.00001
epoch= 70 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 80 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 90 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 100 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 110 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 120 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 130 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 140 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 150 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 160 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 170 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 180 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 190 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 200 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 210 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 220 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 230 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 240 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 250 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 260 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 270 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 280 W= 2.00000 model=4.00000 grad= 0.00000
epoch= 290 W= 2.00000 model=4.00000 grad= 0.00000
End W= 2.000 model=4.000
```

</div>

<!-- הפלט אינו שמור במחברת המקור; הופק בהרצת קוד הספר ב-PyTorch 2.11.0 (C:envsi-rl) ב-14.9.2026. הערך הסופי 2.000000476837158 תואם לפלט השמור בתא 12 של המחברת. -->

נתבונן בפלט. בכל שורה מודפסים המשקל לפני העדכון, ערך ההפסד באותה נקודה (בפלט הוא נקרא `model`) והנגזרת. באיטרציה הראשונה, ב־w=5.5, הנגזרת היא 7, ולכן הצעד הוא 0.1·7=0.7 והמשקל יורד ל־4.8. בנקודה החדשה הנגזרת כבר קטנה יותר, 5.6, ולכן גם הצעד הבא קטן יותר, 0.56. כך זה נמשך: הנגזרת של הפרבולה היא <span dir="ltr">2w − 4</span>, וככל שהמשקל מתקרב ל־2 היא מתקרבת לאפס. בדוגמה זו הנגזרת קטנה בכל איטרציה ב־20% בדיוק, 7, 5.6, 4.48, 3.584 וכן הלאה, ואיתה מתקצרים הצעדים מאליהם, בלי שאיש הורה לאלגוריתם להאט.

אחרי 20 איטרציות הנגזרת היא 0.08, אחרי 40 היא 0.001, ובסביבות איטרציה 70 היא מתאפסת בדיוק המודפס. מכאן ואילך המשקל וההפסד אינם משתנים עוד, ו־230 האיטרציות הנותרות אינן משנות דבר. זו בדיוק התופעה שתיארנו בסעיף "מתי עוצרים?": קבענו מראש 300 איטרציות, ומהפלט אנו רואים שהמינימום הושג הרבה לפני כן.
...
<figure>
<img src="../assets/gradient-descent/loss-parabola-minimum.png" alt="הפרבולה עם כוכב כחול בתחתיתה, בנקודה w=2 ו־loss=4." style="max-width:100%;height:auto;">
<figcaption>הכוכב מסמן את הנקודה שאליה הגיע החיפוש: תחתית הפרבולה.</figcaption>
</figure>
...
האופטימייזר מחליף שני שלבים שכתבנו בעצמנו בלולאה הקודמת. יוצרים אותו באמצעות `torch.optim.SGD` עם רשימת הפרמטרים לעדכון ועם קצב הלמידה. הפעולה `step()` מבצעת את כלל העדכון על כל הפרמטרים שברשימה, במקום שורת העדכון שכתבנו בתוך `torch.no_grad()`. **גם את איפוס הנגזרות האופטימייזר עושה בשבילנו:** הפעולה `zero_grad()` מאפסת את הנגזרות של כל הפרמטרים שברשימה, במקום `w.grad.zero_()` שקראנו לה בעצמנו. גם כאן חובה לקרוא לה בכל איטרציה; אם נשכח, הנגזרות ייצברו מאיטרציה לאיטרציה והצעדים יהיו שגויים.
...
שימו לב להבדל קטן לעומת הלולאה הקודמת: כאן ההדפסה נמצאת אחרי `step()`, ולכן המשקל מודפס אחרי העדכון, ואילו ההפסד והנגזרת חושבו לפניו. התמונה עצמה זהה למה שראינו במימוש בקוד: הנגזרת מתכווצת מאיטרציה לאיטרציה, וכבר אחרי כ־40 איטרציות המשקל התייצב על 2.
...
<figure>
<img src="../assets/sources/ML/converted/4_Gradient_Descent/assets/cell-17-inline-3.png" alt="שלוש פרבולות: קצב למידה נמוך מדי עם צעדים קטנים רבים, קצב מתאים שמגיע במהירות למינימום, וקצב גבוה מדי שבו החצים קופצים מצד לצד ומתרחקים מהתחתית." style="max-width:100%;height:auto;">
<figcaption>שלושה קצבי למידה. משמאל: קצב נמוך מדי (Too low), צעדים קטנים רבים עד המינימום. באמצע: קצב מתאים (Just right), הגעה מהירה לתחתית. מימין: קצב גבוה מדי (Too high), כל צעד מדלג מעל התחתית ומתרחק ממנה, והחיפוש מתבדר.</figcaption>
</figure>

<!-- editorlm-source-ref: [sources/ML/converted/4_Gradient_Descent/notebook.md#L279-L289] -->

<!-- editorlm-source-ref: [sources/ML/pdf/4. Gradient Descent.pdf#L160-L173] -->

...
<a href="20-%D7%A8%D7%A9%D7%AA%D7%95%D7%AA%20%D7%A7%D7%95%D7%A0%D7%91%D7%95%D7%9C%D7%95%D7%A6%D7%99%D7%94.md">→ הקודם</a>
<a class="toc-link" href="../index.md">תוכן העניינים</a>
<a href="22-WandB.md">הבא ←</a>
</nav>

## ג.21 חתול או לא חתול — CNN לסיווג בינארי
...
הבעיה היא סיווג בינארי, כמו ברגרסיה לוגיסטית ובזיהוי הספרה 7, אבל הקלט הוא תמונה צבעונית. לכן נשלב שני דברים שהכרנו בנפרד: את שכבות הקונבולוציה, שיודעות להפיק תכונות מתמונה, ואת נוירון הפלט היחיד עם Sigmoid ו־BCELoss, שיודע לענות כן או לא. לאורך הפרק נשים לב לצורות של הטנסורים — זו הנקודה שבה רוב הטעויות קורות כשעובדים עם נתונים משלנו.
...
להרצה נדרש הקובץ `cats.pth` של הקורס — קובץ PyTorch שבו שמורים, בפורמט שהכרנו בפרק ג.19, מערך התמונות ומערך התוויות. מעלים אותו באמצעות Files ואז Upload בסרגל הצד של Colab; האחסון בסביבת הריצה זמני. הטעינה משתמשת ב־`weights_only=False`, ולכן יש לטעון רק קובץ ממקור מהימן.
...
300 תקופות של קונבולוציה לוקחות זמן, ולא נרצה לחזור עליהן אם סביבת הריצה תתאפס. נשמור את המודל המאומן כולו, כפי שלמדנו בפרק ג.19:
...
הרשת מסווגת נכון כ־79% מתמונות הבדיקה. זה הרבה יותר מניחוש, אבל רחוק מהפסד האפס שראינו באימון — שוב הפער בין שינון להכללה שפגשנו בפרקים הקודמים. הסיבה העיקרית כאן היא כמות הנתונים: 207 תמונות אימון הן מעט מאוד ללימוד מושג מורכב כמו "חתול", בזוויות, צבעים ורקעים שונים. במאגר כה קטן כדאי גם לזכור שכל תמונת בדיקה שווה כמעט 2% מהדיוק, ולכן המספר עצמו אינו מדויק במיוחד.

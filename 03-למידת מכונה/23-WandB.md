...
נניח שאימנו רשת פעמיים: פעם בקצב למידה קטן ופעם בקצב גדול יותר. את תוצאת הריצה האחרונה קל לראות במסוף, אבל כיצד נשווה את שני תהליכי האימון? וכיצד נזכור באילו הגדרות השתמשנו בכל ריצה?

**<span dir="ltr">Weights &amp; Biases — W&amp;B</span>** הוא שירות לתיעוד ניסויים בלמידת מכונה. הקוד שולח אליו את הגדרות הניסוי ואת המדדים שבחרנו לתעד. באתר אפשר לעקוב אחרי הגרפים בזמן האימון ולחזור אליהם לאחר שהסתיים. חבילת Python שמחברת את הקוד לשירות נקראת `wandb`.
...
**Project** מרכז ריצות הקשורות לאותה משימה. **Run** הוא ביצוע מסוים של הניסוי, עם הגדרות ותוצאות משלו. לדוגמה, בפרויקט סיווג תמונות נרשום ריצה אחת לכל קצב למידה שנבדוק. כך נוכל להשוות את השגיאה והדיוק, ולראות מה היו ההגדרות שהובילו לכל תוצאה.
...
האתר מציג את הניסויים בדפדפן; במחשב מתקינים את **חבילת Python** שמדווחת אליו. לשימוש בשירות הענן אין צורך להתקין שרת W&B או תוכנת שולחן עבודה נפרדת.
...
לאחר ההתקנה צריך להזדהות גם מסביבת העבודה המקומית. הכניסה לאתר בדפדפן לבדה אינה מוסרת לתוכנית Python את פרטי ההזדהות. הקישור נעשה באמצעות **API key** — מפתח המאפשר לחבילה לפעול בחשבון שלך.
...
לפני לולאת האימון פותחים ריצה באמצעות `wandb.init`. כאן קובעים לאיזה פרויקט הריצה שייכת, נותנים לה שם, ומתעדים את המידע שיאפשר להבין אותה מאוחר יותר: סוג המודל, הנתונים, קצב הלמידה ומספר האפוקים.
...
את הגדרות הניסוי שומרים בתחילת הריצה. את הערכים שמשתנים — למשל שגיאה ודיוק — מדווחים **במהלך הלולאה**, באמצעות `run.log`:
...
ללא `step` מפורש, מספר צעד הדיווח מתקדם אוטומטית עם הקריאות. הוא אינו בהכרח מספר האפוק. בדוגמה שלנו נשלח פעם אחת בסוף כל אפוק עם `step=epoch`, בסדר עולה. בסיום קוראים `run.finish()` כדי לסיים את הריצה ולהשלים את הדיווח. [דיווח מדדים וציר הצעדים](https://docs.wandb.ai/models/track/log) · [סיום ריצה](https://docs.wandb.ai/models/ref/python/functions/init).
...
פותחים את הקישור לפרויקט שמופיע במסוף, או נכנסים לאתר ובוחרים את הפרויקט `learning-rate-comparison`. בסביבת העבודה **Workspace** מאתרים את שתי הריצות ברשימת **Runs** ומפעילים את הצגת שתיהן. לכל ריצה צבע משלה.

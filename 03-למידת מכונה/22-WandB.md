...
<a href="21-%D7%97%D7%AA%D7%95%D7%9C%20%D7%90%D7%95%20%D7%9C%D7%90%20%D7%97%D7%AA%D7%95%D7%9C.md">→ הקודם</a>
<a class="toc-link" href="../index.md">תוכן העניינים</a>
<a href="23-%D7%94%D7%A9%D7%9C%D7%9E%D7%95%D7%AA%20-%20%D7%A8%D7%92%D7%A8%D7%A1%D7%99%D7%94%20%D7%9C%D7%90%20%D7%9C%D7%99%D7%A0%D7%90%D7%A8%D7%99%D7%AA.md">הבא ←</a>
</nav>

## ג.22 — מעקב והשוואת ניסויים עם <span dir="ltr">W&amp;B</span>

לאורך חלק ג אימנו עשרות רשתות, ובכל פעם בחרנו ערכים: קצב למידה, מספר תקופות, גודל אצווה, מספר נוירונים בשכבה, שיטת נרמול. את הבחירות האלה עשינו בלי הרבה הסבר, אבל בעבודה אמיתית הן לב העניין: אימון רשת הוא תהליך של ניסוי וטעייה, שבו משנים הגדרה, מריצים שוב, ומשווים. לעיתים קרובות משנים כמה הגדרות בבת אחת, הדיוק משתפר, ואי אפשר לומר איזה מהשינויים הוא שעזר. כדי לענות על שאלות כאלה צריך להריץ ניסויים רבים ולשמור סדר ביניהם.
...
<figure>
<a href="../assets/wandb/workspace.png"><img src="../assets/wandb/workspace.png" alt="סביבת העבודה של W&B: רשימת ריצות משמאל וגרפי loss ודיוק מימין"></a>
...
### ג.22.2 — פתיחת חשבון וכניסה לאתר
...
<figure>
<a href="../assets/wandb/signup.png"><img src="../assets/wandb/signup.png" style="width:640px;max-width:100%" alt="מסך הכניסה של W&B, הכולל לשוניות Log in ו-Sign up ואפשרויות הזדהות"></a>
<figcaption>מסך הכניסה והרישום באתר, כפי שנצפה בספטמבר 2026. למשתמש חדש בוחרים בלשונית Sign up.</figcaption>
</figure>

### ג.22.3 — התקנה מקומית ויבוא החבילה
...
### ג.22.4 — קישור המחשב לחשבון: הזדהות מקומית
...
**הזדהות ב־Colab.** ב־Colab אין טרמינל, וגם לא צריך אותו: את שני השלבים מבצעים בתאי קוד של המחברת. בתא הראשון מתקינים, `%pip install wandb`, ובתא שאחריו מריצים את גרסת ה־Python של ההזדהות:
...
התא מדפיס קישור לדף מפתחות ה־API באתר ומציג תיבת קלט. לוחצים על הקישור, מעתיקים את המפתח, מדביקים אותו בתיבה ולוחצים Enter. זה בדיוק מה שהטרמינל עשה במחשב, רק שכאן ההדבקה מתבצעת בתוך המחברת. בשונה מהמחשב, סביבת Colab מתאפסת בכל סשן, ולכן ההתקנה וההזדהות חוזרות בכל פתיחה של המחברת. כדי לא להדביק את המפתח בכל פעם, שומרים אותו ב־**Secrets** של Colab: לוחצים על סמל המפתח בסרגל הצד השמאלי, מוסיפים סוד בשם `WANDB_API_KEY` עם ערך המפתח, ומסמנים "Notebook access". מעתה `wandb.login()` מוצא את המפתח בעצמו ואינו שואל. גם כאן אין להדביק את המפתח בתוך תא קוד, כי המחברת נשמרת ומשותפת.

<!-- editorlm-source-ref: [sources/RL/9.DDQN - Space_Invaders.pptx#L179-L181] -->

### ג.22.5 — אתחול הניסוי ושמירת ההגדרות
...
שימו לב שהערכים במילון אינם חייבים להיות טקסט. `"model"` הוא מחרוזת תיאור, אבל `"learning_rate"` ו־`"epochs"` הם המשתנים עצמם, אותם משתנים שהאימון ישתמש בהם. זו הדרך המומלצת: מגדירים את ההגדרות פעם אחת כמשתנים בראש התוכנית, ומכניסים למילון את המשתנים ולא את המספרים. כשנרצה לנסות קצב למידה אחר נשנה רק את השורה `learning_rate = 0.05`, והשינוי יגיע גם לאימון וגם לתיעוד; את קריאת `init` לא נצטרך לגעת. אם היינו כותבים במילון את המספר 0.05 ידנית, בפעם הראשונה ששוכחים לעדכן אותו התיעוד היה מתאר ריצה שלא התקיימה.
...
שמירת `learning_rate` ב־`config` אינה משנה בעצמה את האופטימייזר. הקוד צריך להשתמש באותו ערך גם באימון. בדוגמה ניגש אל `config["learning_rate"]` בחישוב העדכון. שמירת תיאור המודל במילון גם אינה שמירה של המשקלים שאומנו; לשמירה וטעינה של המשקלים ראו [פרק ג.19](19-שמירה%20וטעינה.md). [הגדרות ניסוי](https://docs.wandb.ai/models/track/config).

#### מה נהוג לשמור ב־config

הכלל פשוט: כל ערך שאם נשנה אותו נקבל ריצה אחרת, שווה לתעד. ברשת אמיתית זו רשימה ארוכה יותר מקצב למידה בלבד. נהוג לשמור לפחות:

- **קצב הלמידה** — `learning_rate`.
- **מספר התקופות** — `epochs`, וכן **גודל האצווה** — `batch_size`; שניהם משפיעים על מספר צעדי העדכון ועל צורת גרף ההפסד.
- **הנתונים** — שם המאגר, ואם היה נרמול או עיבוד מקדים.
- **האופטימייזר** — לא רק קצב הלמידה אלא גם מי הוא: SGD או Adam נותנים ריצות שונות באותו קצב למידה.
- **פונקציית ההפסד**.
- **מבנה הרשת** — כמה שכבות, מאיזה סוג ובאילו גדלים. זה הערך שהכי קל לשכוח, ובלעדיו אי אפשר לדעת אילו שתי ריצות בכלל ניתנות להשוואה.
- **ההתקן** — CPU או GPU, בעיקר כדי להסביר הבדלי זמן ריצה.

את רוב הערכים האלה לא צריך להקליד כטקסט; אפשר לשאול את האובייקטים עצמם. `str(Model)` מחזירה את מבנה הרשת כפי שהוא מודפס במסוף, שכבה אחרי שכבה עם הגדלים, ו־`type(optim).__name__` מחזירה את שם המחלקה של האופטימייזר, למשל `'Adam'`. כך התיעוד נלקח מהאובייקטים שבאמת משמשים לאימון ואינו יכול לסטות מהם. נראה זאת עם רשת הקונבולוציה מפרק ג.20, כשהמודל, פונקציית ההפסד והאופטימייזר כבר מוגדרים:
...
config = {
    ...
    "optimizer": type(optim).__name__,   # 'Adam'
    "loss": type(Loss).__name__,         # 'CrossEntropyLoss'
    "model": str(Model),                 # the network structure
    "device": str(device),               # 'cuda' or 'cpu'
}

run = wandb.init(
    project="cifar10-cnn",
    name=f"{config['optimizer']}-lr-{learning_rate}",
    config=config,
)
```
...
הערך של `"model"` הוא הטקסט שמדפיסה `print(Model)`:
...
**שם הניסוי.** `project` הוא רק המגירה שבה הריצות נאספות; כל ריצה היא ניסוי בפני עצמו, ולכן גם לה נותנים שם ב־`name`. בלי `name` השירות ממציא שם אקראי כמו `sunny-sky-12`, ואחרי עשר ריצות לא נזכור איזו היא איזו. שם טוב נבנה מאותם משתנים שנשמרו ב־config ומספר מה מייחד את הריצה: בדוגמה השם הוא `Adam-lr-0.001`, וכשמשווים גם גדלי אצווה מוסיפים אותם, למשל `f"{config['optimizer']}-lr-{learning_rate}-bs-{batch_size}"`. כך ברשימת הריצות ובמקרא הגרפים רואים מיד מה השתנה בין ניסוי לניסוי, עוד לפני שפותחים את ה־Config. לתיאור חופשי ארוך יותר יש את הפרמטר `notes`, ולסימון קבוצות של ניסויים, למשל כל הריצות עם Adam, את `tags`.

באתר, בעמוד הריצה תחת **Config**, יופיעו כל המפתחות האלה עם ערכיהם. כשנוסיף שכבה לרשת או נחליף את Adam ב־SGD, לא נצטרך לזכור לעדכן דבר: הריצה הבאה תתועד עם המבנה והאופטימייזר החדשים מעצמה, ובהשוואה בין ריצות נוכל לראות במבט אחד במה בדיוק הן נבדלות. גם בתוך הקוד אפשר לקרוא את ההגדרות מהריצה, `run.config["learning_rate"]`, כדי שמקור האמת יהיה אחד.

<!-- editorlm-source-ref: [sources/RL/9.DDQN - Space_Invaders.pptx#L185-L187] -->

### ג.22.6 — דיווח מדדים במהלך האימון
...
### ג.22.7 — דוגמה מלאה: השוואת שני קצבי למידה
...
### ג.22.8 — צפייה בתוצאות והשוואת ריצות
...
<figure>
<a href="../assets/wandb/smoothing.png"><img src="../assets/wandb/smoothing.png" alt="גרף W&B עם נתונים גולמיים בקו בהיר ומגמה מוחלקת בקו כחול כהה"></a>

...
מסנן אחד מזהה תבנית אחת, וזה מעט מאוד. כדי לתאר תמונה צריך לחפש בה במקביל קווים בכיוונים שונים, קצוות, כתמי צבע ועוד. לכן אפשר להפעיל כמה מסננים על אותו קלט, לכל אחד משקלים משלו. כל מסנן מפיק מפת תכונות אחת, ולכן מספר המסננים קובע את מספר ערוצי הפלט. מפות התכונות האלה הן ה"ערוצים" של השכבה הבאה, ממש כפי ששלושת ערוצי הצבע היו הערוצים של השכבה הראשונה.
...
כאן `n_in` הוא גודל הקלט בציר, `k` גודל המסנן, `p` הריפוד ו־`s` הצעד. הנוסחה מחושבת בנפרד לכל ציר, רוחב וגובה; בתמונות ריבועיות התוצאה זהה בשניהם. נבדוק אותה על הדוגמה מסעיף Stride: קלט 5, מסנן 3, ריפוד 0 וצעד 1 נותנים `floor((5+0−3)/1)+1=3`, ועם צעד 2 מתקבל `floor(2/2)+1=2`, בדיוק כפי שראינו.
...
גם ל־Pooling יש חלון וצעד, ולכן גם את גודל הפלט שלו אפשר לחשב באותו אופן. ללא padding, גודל הפלט הוא:
...
`W` הוא גודל הקלט בציר, `F` גודל החלון ו־`S` הצעד. בחלון 2×2 ובצעד 2, המקרה הנפוץ, הפלט הוא בדיוק חצי מהקלט בכל ציר.

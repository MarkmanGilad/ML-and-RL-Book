...
לאחר אימון נרצה לשמור את המודל כדי להשתמש בו בהמשך בלי לאמן אותו מחדש. נלמד שתי דרכים: שמירת המודל כולו ושמירת הפרמטרים בלבד.
...
נכין מודל ודוגמאות אימון שישמשו להדגמת השמירה והטעינה.
...
נטען MNIST וניצור אצוות של 50 תמונות:
...
נציג שש תמונות מהבדיקה ואת התוויות שלהן:
...
נגדיר 784 קלטים, שכבה נסתרת של 200 נוירונים, עשרה פלטים ושתי תקופות אימון:
...
נגדיר את מחלקת הרשת, עם ReLU אחרי השכבה הראשונה:
...
נגדיר CrossEntropyLoss ואופטימייזר Adam:
...
נחשב את אחוז התחזיות הנכונות על קבוצת הבדיקה:
...
נציג תחזיות ותוויות מאצוות הבדיקה הראשונה:
...
נשמור את אובייקט המודל המאומן בקובץ:
...
נטען את המודל ונעביר אותו למצב הערכה. ההגדרה של `ANN_Model` צריכה להיות זמינה בעת הטעינה. `weights_only=False` מאפשרת טעינת אובייקט מלא; משתמשים בה רק לקובץ ששמרנו בעצמנו או שמקורו מהימן, משום שטעינה כזו יכולה להפעיל קוד.
...
נבדוק שהמודל שנטען יכול לשמש לחיזוי. נחשב תחזיות באמצעות `new_model`:
...
אפשר לשמור את הערכים שנלמדו בנפרד מהגדרת מבנה הרשת. הפעולה `state_dict()` מחזירה מילון של פרמטרי המודל. נשמור אותו בנפרד:
...
ניצור רשת חדשה מאותה מחלקה, נטען את המילון ונעביר את הפרמטרים לרשת באמצעות `load_state_dict`:
...
נפעיל את `new_model2` על אצוות הבדיקה:
...
הקבצים בסביבת הריצה של Colab זמניים. לשמירה מתמשכת מחברים את Drive באמצעות אייקון Mount Drive בתפריט הקבצים, ומוודאים שהתיקייה `Colab Notebooks/Models` קיימת ב־MyDrive לפני השמירה:
...
הפקודה כאן שומרת את המודל כולו, משום שנמסר לה `Model`, למרות שם הקובץ `model2.pth`.

...
נמיר את השטח ואת המחיר לטנסורים. הקריאה `reshape(-1,1)` הופכת כל מערך לעמודה, כך שכל שורה מייצגת דירה אחת, כפי ש־`nn.Linear` מצפה לקבל:
...
כעת נגדיר את שלושת המרכיבים של האימון בדיוק כפי שעשינו בפרק הקודם. נשתמש בישר עם משקל והטיה, ב־MSE וב־SGD עם קצב למידה 0.1 — אותו קצב שעבד היטב בדוגמאות הקודמות:

...
<a href="08-%D7%A8%D7%92%D7%A8%D7%A1%D7%99%D7%94%20%D7%A2%D7%9D%20%D7%9B%D7%9E%D7%94%20%D7%A0%D7%A7%D7%95%D7%93%D7%95%D7%AA.md">→ הקודם</a>
<a class="toc-link" href="../index.md">תוכן העניינים</a>
<a href="10-%D7%A0%D7%A8%D7%9E%D7%95%D7%9C%20%D7%A0%D7%AA%D7%95%D7%A0%D7%99%D7%9D.md">הבא ←</a>
</nav>

## ג.9 המחלקה nn.Linear

בפרק הקודם למדנו להתאים לנתונים ישר מהצורה <span dir="ltr">y = w·x</span>. ישר כזה עובר תמיד בראשית הצירים: עבור x = 0 הוא נותן 0, ושום בחירה של w לבדה לא תשנה זאת. אבל בעולם האמיתי זה מצב נפוץ מאוד שהישר חוצה את ציר y בגובה שאינו אפס. מחיר של נסיעה במונית מתחיל מתעריף פתיחה גם לפני הקילומטר הראשון; חשבון חשמל כולל תשלום קבוע שאינו תלוי בצריכה; ובמדידת טמפרטורה יש קשר ישר בין צלזיוס לפרנהייט, אך 0 מעלות צלזיוס אינן 0 פרנהייט. את כל אלה המודל שלנו אינו יכול לתאר, כי הוא "כבול" לנקודה (0, 0).
...
**כלל חשוב: כל שורה היא דוגמה.** PyTorch מצפה לקבל את הנתונים כמו טבלת אקסל: כל **שורה** היא דוגמה אחת, וכל **עמודה** היא נתון אחד על הדוגמה. כאן כל דוגמה היא נקודה, ויש עליה נתון קלט אחד, x, ולכן X הוא טבלה של 100 שורות ועמודה אחת. גם Y בנוי כך: 100 שורות, ובכל שורה התשובה של הדוגמה שבאותה שורה. לכן לכל שורת קלט מתאימה שורת תשובה אחת, ושני הטנסורים בעלי אותה צורה. הכלל הזה ילווה אותנו בכל הספר: גם כשלכל דוגמה יהיו כמה נתונים, וגם כשהדוגמה תהיה תמונה שלמה, כל שורה תישאר דוגמה אחת. ההקפדה על הצורות חשובה: כשנחסר בין התחזית ל־Y בפונקציית ההפסד, שני הטנסורים חייבים להיות באותה צורה, אחרת PyTorch עלול לחסר "כל אחד מכל אחד" ולתת תוצאה שגויה בלי הודעת שגיאה.
...
עד עכשיו כתבנו את המודל ואת פונקציית ההפסד בעצמנו. עבור ישר עם שני פרמטרים זה קל, אך רשת נוירונים יכולה להכיל אלפי פרמטרים, ואיש אינו רוצה להגדיר כל אחד מהם ידנית. לשם כך קיימת הספרייה `torch.nn`, שמספקת אבני בניין מוכנות למודלים; כל אבן בניין כזו נקראת **שכבה — Layer**. PyTorch מספקת מודל לינארי מוכן. `nn.Linear(1, 1, bias=True)` מגדירה קלט אחד ופלט אחד, עם משקל והטיה, ומבצעת את החישוב `wx+b` — בדיוק הפונקציה `Model` שכתבנו למעלה. המספרים בסוגריים הם מספר ערכי הקלט ומספר ערכי הפלט **של דוגמה אחת**, כלומר מספר העמודות בטבלת הקלט ומספר העמודות בטבלת הפלט. `nn.Linear` מצפה לקבל בדיוק את הטבלה שהכנו: שורה לכל דוגמה, ובכל שורה עמודת קלט אחת. בקריאה אחת היא מעבדת את כל 100 השורות בבת אחת ומחזירה טבלה של 100 שורות ועמודת פלט אחת, תחזית לכל דוגמה. בפרקים הבאים נראה שכבות עם קלטים ופלטים רבים, ואז יהיו בשורה יותר עמודות; מבנה השורות לא ישתנה.

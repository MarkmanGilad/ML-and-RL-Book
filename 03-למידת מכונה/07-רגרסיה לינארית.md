...
נגדיר את הנתונים כטנסורים. את המשקל שנאמן נאתחל ל־‎−1.5 עם `requires_grad=True`, ונבחר קצב למידה 0.01:
...
נכין ארבע נקודות. ערכי y מתקבלים מהכפלת x ב־2 ומהוספת מספר אקראי בין 0 ל־1:
...
נחשב את ההפסד עבור 100 משקלים בין ‎−4 ל־9. `W@X` יוצר טבלת תחזיות: שורה לכל משקל ועמודה לכל נקודה; `mean(1)` מחשבת את ההפסד הממוצע לכל משקל.

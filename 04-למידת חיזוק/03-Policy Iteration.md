...
<a href="02-%D7%9E%D7%95%D7%93%D7%9C%20%D7%A1%D7%91%D7%99%D7%91%D7%94%20%D7%A1%D7%95%D7%9B%D7%9F%20%D7%95-MDP.md">→ הקודם</a>
<a class="toc-link" href="../index.md">תוכן העניינים</a>
<a href="04-Value%20Iteration.md">הבא ←</a>
</nav>

## ד.3 — תכנון דינמי: Policy Iteration

**המצגת:** [תכנון דינמי](../../../sources/RL/2.תכנון%20דינמי.pptx) · **קוד:** [הסביבה הנתונה](../assets/rl/code/gridworld.py) · [Policy Iteration](../assets/rl/code/policy_iteration.py)

בלוח קטן שהחוקים שלו ידועים אפשר לחשב אילו מהלכים כדאי לבצע, עוד לפני שמשחקים בפועל. לכל מצב נשמור ערך בטבלה. נתחיל ממדיניות פשוטה, נחשב לאן היא מובילה, ונשפר אותה על סמך התוצאות. השיטה הזו נקראת **תכנון דינמי**: מפרקים חישוב של מסלול ארוך לחישובים קטנים הקשורים זה בזה.

### משוואת בלמן: צעד אחד וההמשך שלו

נניח שהמדיניות בוחרת במצב s את הפעולה a, ושהמודל הידוע מחזיר מצב חדש s′ ותגמול r. את ערך המצב אפשר לפרק לשני חלקים: התגמול שמתקבל עכשיו, והערך של ההמשך כשהוא מוכפל במקדם ההיוון. למושגי הערך וההיוון ראו [פרק המודל](02-מודל%20סביבה%20סוכן%20ו-MDP.md#discount).

<a id="bellman"></a>
<div class="formula" dir="ltr">a = π(s), &nbsp; (s′,r) = model(s,a)<br>V<sup>π</sup>(s) = r + γV<sup>π</sup>(s′)</div>

זו **משוואת בלמן** למדיניות נתונה בסביבה דטרמיניסטית. היא מאפשרת לעדכן תא בטבלה באמצעות תא אחר, במקום לסכום מחדש מסלול שלם בכל פעם. אם המצב הבא סופי, ערכו 0, ולכן נשאר רק התגמול המיידי.
...
למשל, אם הפעולה מגיעה מיד ליעד, נקבל 1. אם קודם צריך לבצע צעד ללא תגמול, ואחריו נגיע למצב שערכו 1, נקבל 0+0.9×1=0.9. צעד נוסף אחורה נותן 0.81. כך המידע על היעד יכול להתפשט בטבלה.

<!-- editorlm-source-ref: [sources/RL/2.תכנון דינמי.pptx#L1-L17] -->

### מתחילים ממדיניות נתונה

נשתמש ב־[לוח 4×4 שהוגדר בפרק הקודם](02-מודל%20סביבה%20סוכן%20ו-MDP.md). נשמור שתי טבלאות: V מכילה מספר לכל מצב, ו־π מכילה פעולה לכל מצב שאינו סופי. נאתחל את הערכים באפס. המדיניות הראשונית יכולה להיות פשוטה מאוד; בשלב הזה היא אינה חייבת להיות טובה.
...
הפונקציה הבאה יוצרת אותה בעזרת קבועי הסביבה:
...
בשלב **הערכת המדיניות** לא משנים את הפעולות שנבחרו. עוברים שוב ושוב על המצבים ומעדכנים את V לפי אותה מדיניות. בכל סריקה מודדים את השינוי הגדול ביותר בערכים. כשהשינוי קטן מן הדיוק שבחרנו, מפסיקים.

תחילה מתקבל ‎−1 מעל תא ההפסד, ו־1 בשני שכני היעד. לאחר מכן מתפשט 0.9 אל המצבים שמגיעים לשכנים האלה לפי המדיניות, ולבסוף 0.81 לתא הימני העליון. בסיום ההערכה הראשונה מתקבלת:
...
העמודות השמאליות אינן מקבלות ערך חיובי רק משום שיש יעד במקום אחר בלוח: המדיניות הנוכחית עדיין אינה מובילה אותן אליו.
...
`old_value` שומר את המספר שלפני העדכון, ו־`delta` מודד עד כמה הטבלה עדיין משתנה. מצבים סופיים נשארים באפס. העדכון נעשה בטבלה עצמה, ולכן מצב שנבדק בהמשך הסריקה עשוי להשתמש בערך שכבר התעדכן באותה סריקה. אין צורך לחכות לסריקה נוספת כדי להשתמש במידע החדש.
...
כעת נשאל בכל מצב אם פעולה אחרת תיתן תוצאה טובה יותר. לכל פעולה חוקית מחשבים את התגמול המיידי ואת ערך ההמשך. בוחרים פעולה עם הציון הגדול ביותר ומשנים את המדיניות בהתאם.

<div class="formula" dir="ltr">π<sub>new</sub>(s) = argmax<sub>a</sub> [R(s,a,s′) + γV<sup>π</sup>(s′)]</div>

`argmax` מחזירה את הפעולה שמביאה למקסימום, ולא את הערך המספרי עצמו. בשלב הזה משתמשים בטבלת הערכים שחושבה עבור המדיניות הקודמת.
...
הפונקציה מחזירה `True` אם לא נדרשה החלפה בשום מצב. כאשר כמה פעולות נותנות אותו ערך, נשמור את הפעולה הקודמת. כך לא נחליף הלוך ושוב פעולות שקולות. התוספת הזעירה `1e-12` מונעת החלפה בגלל הבדלי עיגול מזעריים.

<!-- editorlm-source-ref: [sources/RL/2.תכנון דינמי.pptx#L123-L163] -->

### מעריכים שוב ומשפרים שוב

לאחר שינוי המדיניות, הטבלה עדיין מתארת את המדיניות הישנה. לכן מפעילים שוב הערכת מדיניות, הפעם עבור הפעולות החדשות. הערכים החדשים עשויים לחשוף הזדמנויות לשיפור נוסף.
...
זהו התפקיד של ההחלפה בין השלבים: שינוי המדיניות מאפשר לערכים חדשים להתפשט, והערכים האלה מאפשרים למצוא שינוי נוסף במדיניות.

<!-- editorlm-source-ref: [sources/RL/2.תכנון דינמי.pptx#L165-L319] -->

### Policy Iteration — מחברים את השלבים

האלגוריתם המלא מפעיל הערכה ושיפור עד שהמדיניות יציבה. נתחיל במדיניות שיצרנו בתחילת הפרק ונשתמש באותן שתי פונקציות בכל סבב.
...
בסביבה הסופית שלנו, עם γ=0.9 והערכת מדיניות מדויקת מספיק, ההחלפה החוזרת בין השלבים נותנת מדיניות מיטבית עד לדיוק החישוב. אין פירוש הדבר שיש רק טבלת פעולות מיטבית אחת: לעיתים שתי דרכים שונות מגיעות ליעד באותה תשואה.

...
במילון קיבלנו מצב ופעולה והחזרנו מספר. גם רשת יכולה לקבל קלט ולהחזיר מספר, אלא שהמספר מחושב באמצעות משקלים. נסמן את תחזית הרשת ב־Q(S,A;w), כאשר w מייצג את הפרמטרים הנלמדים.

עדכון של רשומה בטבלה משנה רק את הערך שלה. עדכון משקלי הרשת עשוי לשנות תחזיות עבור קלטים רבים. כך הרשת יכולה להכליל למצבים דומים שלא נלמדו כרשומות נפרדות, אך גם תחזית עבור מצב שכבר הכרנו יכולה להשתנות.

<!-- editorlm-source-ref: [sources/RL/6.DQN.pptx#L5-L11] -->

### הקשר ללמידה מפוקחת

מבחינת אימון הרשת נצטרך קלט, תחזית ויעד מספרי. נחשב loss ונעדכן את המשקלים באמצעות backpropagation ואופטימייזר, כפי שלמדנו בחלק למידת המכונה. ההבדל הוא **מקור היעד**: אין לנו מראש תווית שמציינת את Q הנכון לכל פעולה. נבנה את היעד מהתגמול שהתקבל ומהערכת ההמשך.

אפשר לבחור כמה צורות לקלט ולפלט:
...
בפרק היישום נבחר באפשרות השנייה: מצב ופעולה בקלט, ומספר אחד בפלט. בכל מצב נריץ את כל הפעולות החוקיות ונבחר את זו שקיבלה את הערך הגבוה ביותר.

<!-- editorlm-source-ref: [sources/RL/6.DQN.pptx#L13-L21] -->

### יעד האימון ופונקציית הטעות

נשתמש ביעד של [Q-learning](07-Temporal%20Difference.md#q-learning), כשהערכים מחושבים כעת באמצעות הרשת. בגרסה ראשונית עם רשת אחת, עבור מעבר שאינו סופי:

<div class="formula" dir="ltr">y = R + γ max<sub>a′∈A(S′)</sub> Q(S′,a′;w)<br>loss = [y − Q(S,A;w)]²</div>

התחזית Q(S,A;w) מתייחסת לפעולה שנדגמה. היעד y משלב את התגמול ואת ההמשך החמדני במצב הבא. בזמן צעד האימון נתייחס ליעד כקבוע ולא נעביר דרכו גרדיאנט. הגרדיאנט יחושב דרך התחזית שאנחנו מנסים לקרב אליו.

לא מעדכנים את פלט הרשת באמצעות השמה, כפי שעשינו בתא של מילון. מחשבים את הטעות, מפעילים `backward` ומעדכנים את המשקלים. עבור אצווה של דוגמאות משתמשים בממוצע ריבועי השגיאות, MSE.

<!-- editorlm-source-ref: [sources/RL/6.DQN.pptx#L29-L39] -->

### האלגוריתם הראשוני ומצב סופי

תחילה אפשר לחשוב על אימון ישירות מכל מעבר: בוחרים פעולה באמצעות ε-greedy, מקבלים מצב הבא ותגמול, מחשבים את היעד ואת ה־loss ומבצעים עדכון משקלים. ממשיכים כך עד סוף המשחק ומתחילים משחק חדש.
...
מעברים עוקבים באותו משחק קשורים זה לזה: מצב אחד נוצר מהקודם, והפעולות נבחרו לפי אותה מדיניות. אם נעדכן רק על פי המעבר האחרון, האימון יתבסס בכל רגע על מקטע צר של ההתנסות.

נשמור מעברים במאגר בשם **Replay Buffer**. כל רשומה מכילה `(state, action, reward, next_state, done)`. לאחר איסוף מעברים נדגום מהם אצווה אקראית ונאמן עליה את הרשת.
...
נשאר קושי נוסף. באימון מסווג תמונות, למשל, התווית של אותה תמונה נשארת קבועה. כאן שינוי משקלי הרשת משנה גם את תחזית Q וגם את היעד שבנינו בעזרת אותה רשת. אנחנו מנסים להתקרב למספר שזז בזמן שאנחנו מתאמנים.

לכן נחזיק שתי רשתות בעלות אותו מבנה:
...
באתחול מעתיקים את משקלי הראשית למטרה. בהמשך, אחת לפרק זמן קבוע, מעתיקים אותם שוב. כך היעדים אינם משתנים בעקבות כל צעד גרדיאנט. עדיין נדרש ללמוד מתוך דגימות; רשת המטרה אינה מקור של תשובות אמת.

<a id="dqn-target"></a>
<div class="formula" dir="ltr">y = R &nbsp; if done<br>y = R + γ max<sub>a′∈A(S′)</sub> Q(S′,a′;w⁻) &nbsp; otherwise</div>

ב־DQN רגיל רשת המטרה גם קובעת איזו פעולת המשך היא הטובה ביותר וגם נותנת את הערך שלה. בחירת פעולת ההתנסות במשחק ממשיכה להיעשות ברשת הראשית עם ε-greedy.

<!-- editorlm-source-ref: [sources/RL/6.DQN.pptx#L59-L65] -->

### DQN — מחברים את הרכיבים

הסדר המלא כולל איסוף נתונים, עדכון הרשת הראשית והעתקה תקופתית לרשת המטרה:
...
כאן יחידת הסנכרון היא **אפיזודות שהסתיימו**; זה גם המימוש שבו נשתמש בפרק איקס עיגול. אם בוחרים במקום זאת C צעדי אימון, זו הגדרה אחרת של התדירות, וצריך לציין אותה במפורש.

כעת יש לנו את כל רכיבי DQN. לפני שנממש אותם על המשחק, נבחן שינוי קטן וממוקד בחישוב היעד: הפרדת בחירת פעולת ההמשך מהערכתה ב־DDQN.

...
המשתנה `device` מקבל את הערך `cuda` אם יש כרטיס גרפי זמין, ו־`cpu` אחרת. כשהקוד מעביר רשת או טנסור אל `device`, אותו קוד רץ נכון בשני המקרים. הדוגמאות בחלק ד נשארות על CPU כדי שכל תלמיד יוכל להריץ אותן; מי שהתקין את גרסת CUDA יוכל להוסיף את השורה הזו ולהאיץ את האימון בפרקים שעוסקים ברשתות נוירונים, החל מפרק ד.11.

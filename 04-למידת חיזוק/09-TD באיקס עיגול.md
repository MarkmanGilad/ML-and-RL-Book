...
**המצגת:** [TD באיקס עיגול](../../../sources/RL/5.1%20Temporal%20Difference-TicTacToe.pptx) · **קוד:** [אימון SARSA](https://github.com/MarkmanGilad/book/blob/main/assets/rl/code/sarsa_tictactoe.py) · [סוכן AfterState](https://github.com/MarkmanGilad/book/blob/main/assets/rl/code/afterstate_tictactoe.py) · [כל קובצי הקוד בגיטהב](https://github.com/MarkmanGilad/book/tree/main/assets/rl/code)
...
$$
\begin{gathered} \text{after\_state} = \text{result\_of\_our\_action}(\text{state}, \text{action}) \\ Q(\text{state}, \text{action}) = V(\text{after\_state}) \end{gathered}
$$
...
$$
V(\text{after\_state}) \leftarrow V(\text{after\_state}) + \alpha \left[ R + \gamma V(\text{next\_after\_state}) - V(\text{after\_state}) \right]
$$

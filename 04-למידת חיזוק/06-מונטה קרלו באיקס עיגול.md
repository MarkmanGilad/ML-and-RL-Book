...
טבלת Q תהיה מילון. המפתח הוא זוג המכיל מצב ופעולה, והערך הוא מספר:
...
הסוכן הטבלאי בקובץ המצורף כולל `get_Q`, שמחזירה את הערך מהמילון או 0, ו־`get_action`, שבוחרת לפי ε-greedy. מימוש הבחירה והדעיכה נמצא ב־[פרק מונטה קרלו](05-מונטה%20קרלו.md#epsilon-decay). כעת נשתמש בו כדי ליצור משחק שלם:

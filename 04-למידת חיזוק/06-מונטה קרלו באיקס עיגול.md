...
<a href="05-%D7%9E%D7%95%D7%A0%D7%98%D7%94%20%D7%A7%D7%A8%D7%9C%D7%95.md">→ הקודם</a>
<a class="toc-link" href="../index.md">תוכן העניינים</a>
<a href="07-Temporal%20Difference.md">הבא ←</a>
</nav>

## ד.6 — מונטה קרלו באיקס עיגול — טבלת Q וקוד האימון
...
נבנה סוכן שמשחק X, מתנסה במשחקים מול יריב אקראי ומעדכן את ההערכות שלו אחרי כל משחק. האלגוריתם כבר הוסבר ב־[מונטה קרלו](05-מונטה%20קרלו.md#mc-update). כאן נחבר אותו לקוד: נחליט איך לשמור מצב–פעולה במילון, נאסוף אפיזודה ונעדכן את הזוגות שהופיעו בה.

### ממשק המשחק הדרוש לאימון

המשחק כבר נתון. אין צורך בגרפיקה כדי לאמן את הסוכן: הוא מקבל מצב, בוחר משבצת חוקית ומבקש מהסביבה לבצע את המהלך. בקובץ המצורף `State.board` הוא tuple של תשעה תאים לפי סדר השורות: 1 עבור X, ‎−1 עבור O ו־0 לתא ריק. `State.player` הוא השחקן שתורו כעת. פעולה מיוצגת בזוג `(row, col)`, כשמספור השורות והעמודות מתחיל באפס.
...
צעד למידה אחד יכלול את הפעולה של X ואת תגובת O, אם המשחק לא הסתיים קודם. כך המצב הבא של צעד למידה הוא מצב שבו X שוב צריך לבחור, או מצב סופי. את פונקציית הדגימה נציג אחרי מבנה הטבלה.

### מילון של מצב–פעולה וערך

כדי למצוא את ההערכה של פעולה במצב מסוים, נשמור את טבלת Q כמילון. המפתח הוא זוג המכיל מצב ופעולה, והערך הוא מספר:
...
מילון בפייתון מבוסס על **טבלת גיבוב — Hash Table**. במקום לחפש מפתח ברשימה מתחילתה ועד סופה, מחשבים ממנו מספר באמצעות פונקציית `hash`, ומשתמשים במספר כדי למצוא מקום מתאים בטבלה.
...
גם מחרוזת יכולה לעבור גיבוב. בספר טלפונים, למשל, נרצה שהביטוי `phones["Dana"]` ימצא את המספר של דנה בלי להשוות את השם לכל השמות ששמורים בספר.
...
כאשר המפתחות מתפזרים היטב בטבלה ויש בה מקום מתאים, חיפוש, הכנסה ומחיקה הם O(1) בממוצע. במקרה הגרוע, כאשר נדרשות בדיקות רבות בגלל התנגשויות, הם יכולים להיות O(n). הסימון מתייחס למספר המפתחות n; הוא אינו אומר שכל פעולה במחשב לוקחת בדיוק אותו זמן.

<!-- editorlm-source-ref: [sources/RL/4.1. איקס עיגול.pptx#L27-L39] -->

### מה עושים כששני מפתחות מגיעים לאותו מקום?

בדוגמת המודולו, גם 23 וגם 43 מגיעים לאינדקס 3. זו **התנגשות**. אין פירושה שהמפתחות שווים או שעלינו למחוק את אחד הערכים. צריך לשמור דרך להבחין ביניהם.
...
אם שני אובייקטים מתארים בדיוק את אותו מצב, אנחנו רוצים שיגיעו לאותה רשומה ב־Q. אין טעם ללמוד מחדש את אותו לוח רק משום שנוצר עבורו אובייקט חדש בזיכרון.
...
`__hash__` עוזרת למילון למצוא את מקום החיפוש; `__eq__` בודקת אם אכן מדובר באותו מצב. מצבים שווים חייבים לקבל אותו hash, אבל hash שווה אינו מחייב שמצבים יהיו שווים, בגלל האפשרות להתנגשות.
...
נציג פונקציה שמרכזת צעד למידה אחד. היא אינה מחליטה מה הפעולה של X; זו אחריות הסוכן. היא מבצעת את הפעולה שהתקבלה, בודקת סיום, ואם צריך — מגרילה את תגובת היריב.
...
אם X ניצח או מילא את הלוח לתיקו, אין לתת ליריב תור נוסף. אם המשחק נמשך, ההחזרה השנייה כוללת גם את תגובת O. ניצחון של O נותן ‎−1, בהתאם לנקודת המבט של הסוכן שאנחנו מאמנים.
...
כעת נאסוף משחק שלם שישמש לעדכון הטבלה. הסוכן הטבלאי בקובץ המצורף כולל `get_Q`, שמחזירה את הערך מהמילון או 0, ו־`get_action`, שבוחרת לפי ε-greedy. מימוש הבחירה והדעיכה נמצא ב־[פרק מונטה קרלו](05-מונטה%20קרלו.md#epsilon-decay). כעת נשתמש בו כדי ליצור משחק שלם:
...
לאחר סיום המשחק נפעיל את [עדכון מונטה קרלו](05-מונטה%20קרלו.md#mc-update) על הרשומות שנאספו:
...
`reversed` מאפשרת להתחיל בצעד האחרון בלי למחוק את הרשימה. באפיזודה שניצחה, הצעד האחרון יקבל יעד 1; אם כל התגמולים הקודמים היו 0, הצעד שלפניו יקבל 0.95. בהפסד הסימן שלילי. בתיקו כל התשואות כאן יהיו 0, ועדיין נעדכן את הערכים אם הם היו שונים מאפס.

<!-- editorlm-source-ref: [sources/RL/4. מונטה קרלו.pptx#L99-L105] -->

### לולאת האימון ובדיקת הסוכן

נחזור על התהליך 200,000 פעמים, עם α=0.01 ו־γ=0.95. אלה פרמטרי מאמן מונטה קרלו בענף הפתרון. ε יתעדכן לפי הפונקציה המעריכית שהוסברה בפרק הקודם.
...
מספרי הזרעים מאפשרים לשחזר את בחירות המחוללים בהרצה חוזרת באותה סביבה. נשמור מחולל נפרד לסוכן וליריב. לאחר האימון נבדוק את הסוכן ללא חקירה וללא עדכונים ל־Q:
...
זו תוצאה של 1,000 משחקי בדיקה מול היריב האקראי, לאחר 200,000 משחקי אימון עם הזרעים המוצגים. היא אינה הוכחה שהסוכן אינו יכול להפסיד מול יריב אחר. אפשר לבדוק סוכן חדש באותה דרך ולבחון כיצד הפרמטרים משפיעים על התוצאות, בלי לערב את הגרפיקה של המשחק.

...
נקודה חשובה: מנקודת מבטו של הסוכן הלומד, תגובת היריב היא חלק מ"הסביבה", בדיוק כמו החלקה על כביש חלק בדוגמה מפרק ד.2. לכן צעד למידה אחד יכלול את הפעולה של X ואת תגובת O, אם המשחק לא הסתיים קודם. כך המצב הבא של צעד למידה הוא מצב שבו X שוב צריך לבחור, או מצב סופי. את פונקציית הדגימה נציג אחרי מבנה הטבלה.
...
האפיזודה בידינו, ועכשיו מגיע שלב הלמידה. לאחר סיום המשחק נפעיל את [עדכון מונטה קרלו](05-מונטה%20קרלו.md#mc-update) על הרשומות שנאספו. הפונקציה עוברת על הרשומות מהסוף להתחלה, צוברת את התשואה G ומזיזה כל ערך Q לכיוונה:

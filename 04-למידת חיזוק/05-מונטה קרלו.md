...
מה עושים עם הרשימה הזאת? באיקס־עיגול התגמול מגיע רק בסוף — ניצחון, הפסד או תיקו — וכל המהלכים שלפניו קיבלו 0. כדי לייחס את התוצאה גם למהלכים המוקדמים, נחשב לכל צעד את התשואה שלו, כלומר מה יצא ממנו בסופו של דבר. נסמן כאן ב־R<sub>t</sub> את התגמול שהתקבל בעקבות הפעולה A<sub>t</sub>. לאחר סיום האפיזודה נוכל לחשב את התשואה מכל נקודה ברצף באמצעות [סכום התגמולים המהוון](02-מודל%20סביבה%20סוכן%20ו-MDP.md#discount) שהוגדר בפרק המודל.

...
נשתמש ב־[DQN רגיל](09-DQN.md#dqn-target): רשת המטרה תבחר ותעריך את פעולת ההמשך שביעד. ההפרדה של DDQN הוסברה בפרק הקודם; כאן נוכל לזהות במימוש בדיוק היכן מחשבים את המקסימום.

### הממשק שהאימון מקבל מהמשחק

נשתמש באותו [ממשק איקס עיגול](06-מונטה%20קרלו%20באיקס%20עיגול.md#game-interface). המצב שומר לוח ותור, והפעולה היא `(row, col)`. הפונקציה [sample_step](06-מונטה%20קרלו%20באיקס%20עיגול.md#sample-step) מבצעת את פעולת X ואת תגובת היריב, אם נדרשת, ומחזירה `(next_state, reward, done)`.

הרשת תקבל את תשעת תאי הלוח בלבד. זה מספיק לדוגמה הזאת משום שכל מצב החלטה שנאמן עליו הוא של X; התגמול תמיד מנקודת מבטו. אם מאמנים בהמשך סוכן לשני התורים, צריך להתאים גם את הייצוג ואת משמעות הערכים.

<!-- editorlm-source-ref: [sources/RL/7.DQN_tic_tac_toe.pptx#L1-L15] -->

### ReplayBuffer — שומרים חמישה שדות ודוגמים אצווה

נממש את [מאגר המעברים](09-DQN.md#replay) באמצעות `deque`. הפרמטר `maxlen` מגביל אותו ל־10,000 רשומות; כשהוא מלא והכנסנו רשומה חדשה, הישנה ביותר תצא אוטומטית.

כל מעבר יישמר כחמישה טנסורים. חשוב לשמור גם את הפעולה, משום שהרשת שלנו מקבלת מצב **ופעולה** כדי לחשב Q. כל טנסור של דוגמה אחת כולל ממד אצווה של 1, כך שאפשר לחבר דוגמאות בעזרת `vstack`.
...
`zip(*batch)` מפרידה את רשומות האצווה לפי שדות: כל המצבים יחד, כל הפעולות יחד וכן הלאה. לאחר האיחוד, עבור אצווה בגודל B יתקבלו הצורות הבאות:
...
תשעת תאי הלוח ושתי קואורדינטות הפעולה יוצרים קלט בן 11 מספרים. נשתמש בשתי שכבות חבויות של 128 ו־64 יחידות, ובפלט יחיד:

<div class="formula" dir="ltr">11 → 128 → 64 → 1</div>

אחרי כל שכבה חבויה נפעיל ReLU. בשכבת הפלט לא נוסיף אקטיבציה: ערך Q אינו הסתברות, ויכול להיות שלילי. כל הרשת והטנסורים בדוגמה נמצאים על CPU.
...
הסוכן מחזיק את הסביבה ואת הרשת הראשית. בזמן האימון הוא יכול לחקור; בזמן בדיקה הוא יבחר תמיד לפי תחזית הרשת. נשמור את הדגל בשם `training`, ואת המתודה לשינוי המצב בשם `set_training`, כדי שלא נסתיר מתודה באמצעות שדה בעל אותו שם.
...
`epsilon_at` היא פונקציית הדעיכה המעריכית מ־[פרק מונטה קרלו](05-מונטה%20קרלו.md#epsilon-decay). כאן נשתמש ב־`decay=5000`, לפי קובץ הסוכן של DQN. בתחילת האימון ε=1 ובהמשך הוא מתקרב ל־0.01. בזמן בדיקה ε=0 בלי תלות במספר האפיזודה.
...
`no_grad` מונע בניית גרף גרדיאנטים בזמן בחירת פעולה. `train(False)` מעביר את הרשת למצב הערכה; הוא אינו תחליף ל־`no_grad`. שני המנגנונים משמשים למטרות שונות.

<!-- editorlm-source-ref: [sources/RL/7.DQN_tic_tac_toe.pptx#L43-L49] -->

### ערכי ההמשך של אצווה

לפני לולאת האימון נכתוב פונקציה לחישוב ערכי ההמשך. כל שורה ב־`next_states` מתארת לוח אחר. במעבר סופי נשאיר 0; במעבר שאינו סופי נמצא את המשבצות הפנויות וניקח את הערך המרבי של **רשת המטרה** בלבד.
...
`nonzero` מחזירה כאן את קואורדינטות התאים הריקים. איננו צריכים ליצור מחדש את כל אובייקט המשחק כדי לקבל אותן. בדיקת `done` מקדימה את החיפוש, כך שלוח סופי אינו דורש פעולה מדומה או חישוב מקסימום על רשימה ריקה.

זה המקום שמבדיל את המימוש מ־DDQN: כאן המקסימום מחושב כולו במטרה. ב־DDQN היינו מוצאים תחילה פעולה חמדנית בראשית, ורק אז מעריכים אותה במטרה, כפי שהוסבר בפרק הקודם.
...
`EPOCHS` הוא מספר המשחקים, `C` הוא מספר **האפיזודות שהסתיימו** בין סנכרוני המטרה, ו־`BATCH_SIZE` הוא מספר המעברים בכל עדכון. נשתמש ב־SGD כדי לעדכן את הרשת הראשית.

באתחול ניצור שני אובייקטים נפרדים ונעתיק את משקלי הראשית למטרה. השמה כגון `target = Q` לא הייתה יוצרת רשת עצמאית. האופטימייזר יקבל רק את פרמטרי Q; רשת המטרה לא תלמד דרך גרדיאנטים.
...
המאמן הבא כולל את האתחול ואת לולאת המשחקים. המחלקות והפונקציות שהוצגו נשמרות בקבצים המקושרים בראש הפרק; קובץ המאמן כולל את הייבואים הנדרשים.
...
איפוס הגרדיאנטים מקדים את `backward`, ואחריו `step` משנה את משקלי Q. ערכי ההמשך חושבו ללא גרדיאנט, ולכן אין עדכון עקיף של רשת המטרה. `losses` שומרת מספר אחד לכל עדכון, לצורך עיון במהלך האימון.
...
בסיום האימון נשמור את משקלי הרשת הראשית. קובץ המאמן עושה זאת ל־`dqn_weights.pth` בתיקיית הקוד:
...
Loss נמוך אינו לבדו מדד ליכולת משחק: הוא מודד התאמה ליעדים שנבנו מהדגימות ומהרשת. ספירת התוצאות מול היריב בודקת את ההתנהגות בפועל. גם תוצאה טובה מול יריב אקראי אינה מוכיחה משחק מיטבי מול כל יריב.

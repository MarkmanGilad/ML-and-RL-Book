...
חריגה שלא נתפסה בפונקציה עוברת לקוד שקרא לה, וכך הלאה במעלה שרשרת הקריאות, עד שמישהו תופס אותה או עד שהתוכנית נעצרת. כאן החריגה נוצרה בתוך `positive_age` ונתפסה ב־`except` שמחוץ לה. `as error` נותנת שם לאובייקט החריגה כדי שאפשר יהיה לקרוא את ההודעה שלו, זו שנכתבה בסוגריים של `raise`.

...
בניסיון השני מתקיים התנאי `attempts == 2`, מודפס `Found` והלולאה נעצרת ב־`break`. בלוק ה־`else` לא יבוצע, משום שהלולאה הסתיימה באמצעות `break`. אילו לא היה נמצא דבר בשלושת הניסיונות, הלולאה הייתה מסתיימת כי `attempts < 3` נעשה שקרי, ואז היה מודפס `Not found`. [פירוט על לולאות ו־else](https://docs.python.org/3/tutorial/controlflow.html).

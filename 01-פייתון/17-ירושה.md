...
**שם המחלקה המורישה נכתב בסוגריים בכותרת המחלקה החדשה.** אם לא מגדירים אתחול חדש, אפשר להשתמש באתחול שהתקבל בירושה.
...
כשמוסיפים `__init__` למחלקה יורשת, קוראים במפורש לאתחול המוריש אם זקוקים לו. **`super().__init__(...)` מפעילה אותו; אין להעביר `self` בקריאה הזאת.**
...
אחרי הגדרת `ComputerPlayer` עם המתודה החדשה, אפשר להפעיל `describe` על שחקנים משני הסוגים:

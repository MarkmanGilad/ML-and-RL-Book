...
<a href="16-%D7%9E%D7%97%D7%9C%D7%A7%D7%95%D7%AA.md">→ הקודם</a>
<a class="toc-link" href="../index.md#book-toc">תוכן העניינים</a>
<a href="18-%D7%A7%D7%91%D7%A6%D7%99%D7%9D.md">הבא ←</a>
</nav>

## א.17 — ירושה והכלה

ירושה מאפשרת להגדיר מחלקה חדשה על בסיס מחלקה קיימת. המחלקה החדשה יכולה להשתמש בפעולות שקיבלה ממחלקת הבסיס, להוסיף יכולות משלה ולהתאים פעולה קיימת לצרכיה.

למשל, לשחקן אנושי ולשחקן ממוחשב יכולים להיות שם ופעולת תיאור משותפים, אף שהדרך שבה כל אחד בוחר מהלך שונה. את החלק המשותף נגדיר במחלקת שחקן, ואת ההבדלים נוכל להגדיר במחלקות היורשות ממנה.

**[פתיחת המחברת ב־Colab](https://colab.research.google.com/drive/1AE5GPupf-T8vgzzohtv0O3k-5xh0g058?usp=share_link) · [השיעור וההרצאה באתר הקורס](https://webprogramming.azurewebsites.net/Pages/Python/Inheritance.aspx)**
...
ניצור סוג של שחקן שמקבל את הנתונים והפעולות של מחלקת השחקן הקיימת. **שם המחלקה המורישה נכתב בסוגריים בכותרת המחלקה החדשה.** אם לא מגדירים אתחול חדש, אפשר להשתמש באתחול שהתקבל בירושה.
...
**`pass` היא פקודה שאינה עושה דבר**, ומשמשת כאן כגוף חוקי למחלקה שאין בה עדיין תוספות.

<!-- editorlm-source-ref: [sources/Python/Python_Classes_Inheritance.pptx#L45-L47] -->

### הרחבת האתחול באמצעות super

מחלקה יורשת עשויה להזדקק לנתונים נוספים, לצד אלה שכבר מוגדרים במחלקת הבסיס. כשמוסיפים `__init__` למחלקה יורשת, קוראים במפורש לאתחול המוריש אם זקוקים לו. **`super().__init__(...)` מפעילה אותו; אין להעביר `self` בקריאה הזאת.**
...
<!-- editorlm-source-ref: [sources/Python/converted/1.Python_Basic/notebook.md#L673-L719] -->

### דריסת מתודה

כדי לשנות פעולה שהתקבלה בירושה, **מגדירים במחלקה היורשת מתודה באותו שם**. אפשר לקרוא למימוש המקורי באמצעות `super()` ולהרחיב את התוצאה.
...
<!-- editorlm-source-ref: [sources/Python/converted/1.Python_Basic/notebook.md#L727-L758] -->
...
השחקן האנושי משתמש במתודה שירש, והשחקן הממוחשב במתודה שדרס. לקוד הקורא יש אותה צורת פנייה, אך התוצאה נקבעת לפי האובייקט. זו המחשה של **פולימורפיזם — רב־צורתיות**. [ירושה ודריסת מתודות בתיעוד Python](https://docs.python.org/3/tutorial/classes.html#inheritance).

<!-- editorlm-source-ref: [sources/Python/converted/1.Python_Basic/notebook.md#L727-L758] -->

### ירושה מרובה

אפשר לרשת מכמה מחלקות: **מפרידים בין שמותיהן בפסיקים**. בדוגמה כל מחלקה מספקת פעולה אחרת.
...
כאשר אותו שם מופיע בכמה מחלקות, פייתון משתמשת בסדר חיפוש שנקבע לפי הירושה (MRO). **`super()` ממשיכה לפי סדר זה**; בירושה מרובה היא אינה בהכרח פנייה למחלקה אחת קבועה.

<!-- editorlm-source-ref: [sources/Python/Python_Classes_Inheritance.pptx#L63-L65] -->
### ירושה לעומת הכלה

תלמיד הוא אדם — זה יחס המתאים לירושה. כיתה מכילה תלמידים — הכיתה עצמה אינה תלמיד. הכלה ממומשת באמצעות מאפיין שמפנה לאובייקט אחר או לרשימת אובייקטים:
...
<!-- editorlm-source-ref: [sources/אוניברסיטה פתוחה/converted/12-תכנות מונחה עצמים/notebook.md#L744-L890] -->

...
קבוצה מתאימה כשחשוב לדעת אילו ערכים קיימים באוסף או אם ערך מסוים נמצא בו. אין לאיבריה אינדקסים, ולא מסתמכים על סדר הופעתם; אפשר להוסיף ולהסיר איברים לפי ערכם.
...
נייצג את החדרים שבהם ביקר השחקן באמצעות מספריהם. `add` מוסיפה מספר לקבוצה, ו־`remove` מסירה אותו. הוספת מספר שכבר נמצא בקבוצה אינה משנה אותה; `in` בודקת אם המספר קיים.
...
**קבוצה ריקה נכתבת `set()`; הסימון `{}` יוצר מילון ריק.** `remove` דורשת שהאיבר יהיה בקבוצה. `discard` מסירה אותו אם הוא קיים, ללא שגיאה אם אינו קיים.

<!-- editorlm-source-ref: [sources/Python/converted/2.Python_Data_Struct/notebook.md#L668-L689] -->
...
לא מסתמכים על סדר הסריקה של הקבוצה. אם רוצים גם סדר וגם כפילויות, שומרים את הרשימה המקורית.

<!-- editorlm-source-ref: [sources/אוניברסיטה פתוחה/converted/11-מבני נתונים מתקדמים/notebook.md#L540-L574] -->

### איזה מבנה מתאים למשימה?

| הצורך | המבנה המתאים | דוגמה |
| --- | --- | --- |
| רצף שאפשר לשנות ולהרחיב | `list` | רשימת ציונים |
| רצף שאיבריו אינם מוחלפים אחרי יצירתו | `tuple` | זוג קואורדינטות |
| התאמה בין מפתח לערך | `dict` | שם ומספר טלפון |
| אוסף ערכים בלי כפילויות | `set` | מספרי חדרים שבהם ביקרנו |

ב־list וב־tuple פונים לפי אינדקס; במילון לפי מפתח; בקבוצה בודקים שייכות לפי ערך. [מבני נתונים בתיעוד Python](https://docs.python.org/3/tutorial/datastructures.html).

<!-- editorlm-source-ref: [sources/Python/Python_Data_Strucrures.pptx#L15-L97] -->

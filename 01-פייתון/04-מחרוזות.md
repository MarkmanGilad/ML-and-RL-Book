...
האינדקס הראשון הוא `0`. אינדקס שלילי סופר מהסוף: `-1` הוא התו האחרון. `len` מחזירה את מספר התווים.
...
בכתיבה `text[start:stop]`, ההתחלה נכללת והסיום אינו נכלל. השמטת התחלה פירושה מתחילת הטקסט; השמטת סיום פירושה עד סופו.
...
המספר השלישי בחיתוך הוא הצעד; `2` בוחר כל תו שני. `text[::-1]` יוצר טקסט בסדר הפוך.

### מחרוזת אינה משתנה במקום

אי אפשר לבצע `name[0] = "X"`. פעולות כגון `replace`, `upper` ו־`lower` מחזירות מחרוזת חדשה; כדי לשמור אותה צריך לבצע השמה.
...
`strip` מסירה רווחים בקצוות; היא אינה מוחקת רווחים שבין המילים. הנקודה בכתיבה `text.lower()` מפעילה פעולה על הטקסט.

### חיפוש ובדיקות

`in` בודקת אם קטע טקסט מופיע במחרוזת. `find` מחזירה את האינדקס הראשון שבו נמצא הקטע, או `-1` אם אינו נמצא. החיפוש רגיש לאותיות גדולות.
...
`isdigit` בודקת תווי ספרות; היא אינה בדיקת תקינות כללית למספר שלילי או עשרוני. `isalpha` בודקת אותיות, גם בשפות שאינן אנגלית.

### תווים מיוחדים

`\n` מסמן ירידת שורה, `\t` טאב, ו־`\\` לוכסן הפוך. אפשר להקיף טקסט בגרשיים כדי לכלול בו מירכאות, או להפך. תווים מיוצגים לפי Unicode; הפונקציות `ord` ו־`chr` ממירות בין תו למספר המייצג אותו.

...
כל זוג איברים תואמים ב־`x` וב־`y` מייצג נקודה. כאן מחברים את `(1, 2)` ואת `(3, 9)`. **שתי הרשימות צריכות להיות באותו אורך.** `figure()` פותחת מסגרת חדשה; `show()` מציגה את התרשים.
...
![קו אדום המחבר ארבע נקודות לפי סדר הנתונים](../assets/sources/Python/converted/4.PyPlot/assets/cell-9-output-2-2.png)

האיור הוא הפלט השמור במחברת הקורס עבור אותם נתונים.
...
![שני קווים עם כותרת, שמות לצירים, גבולות ומקרא](../assets/sources/Python/converted/4.PyPlot/assets/cell-19-output-1-2.png)
...
כאן יש שורה אחת ושתי עמודות. `tight_layout()` מתאימה את הריווח בין התרשימים והכותרות.
...
![הגרף השמור במחברת: איקס בריבוע כפול קוסינוס איקס](../assets/ou/function.png)
...
![ארבע עמודות בגבהים 4, 7, 1 ו־8 — פלט שמור מהמחברת](../assets/ou/bar.png)
...
בשלושת הטווחים האחרונים יש בהתאמה 3, 4 ו־1 ערכים. במחברת מודגמת גם היסטוגרמה של נתונים אקראיים; התרשים המדויק שלה עשוי להשתנות בין הרצות. בפרק ניתוח הנתונים נראה היסטוגרמת גילים אמיתית מתוך Titanic.

...
סביבת עבודה היא המקום שבו כותבים את הקוד, מריצים אותו ורואים את התוצאה. בחלק זה נשתמש ב־Google Colab, סביבת עבודה שפועלת בדפדפן ומריצה את הקוד בשרתים של Google.
...
לחצו בתוך תא קוד ריק והקלידו שורה אחת:
...
פתחו את תיקיית המחברות הציבוריות, בחרו מחברת לפי הנושא ושמרו עותק ב־Drive שלכם לפני השינוי. עבדו בעותק האישי. במחברת חדשה או לאחר חיבור מחדש, הריצו את התאים מלמעלה למטה: תא מאוחר עשוי להשתמש במשתנה או בפונקציה שהוגדרו קודם. שמירת קובץ המחברת אינה שומרת את הזיכרון של סביבת הריצה.

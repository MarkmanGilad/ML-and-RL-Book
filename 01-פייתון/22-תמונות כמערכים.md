...
צורת המערך מתארת את ממדי התמונה ואת מספר ערוצי הצבע שלה. נבדוק זאת באמצעות דוגמת תמונת הקמפוס במחברת 14. לאחר הורדת קובץ הליווי בנתיב המופיע במחברת, הקוד טוען אותו בעזרת `Image` מספריית Pillow:
...
זהו הגודל השמור לדוגמת הקמפוס: 500 שורות, 750 עמודות ושלושה ערוצים. לתמונה אחרת יכולה להיות צורה אחרת. `uint8` שומר מספרים שלמים בתחום 0–255.
...
כדי להציג תמונה בגווני אפור, נייצג כל פיקסל בערך בהירות יחיד. בדוגמה נחשב לשם כך את ממוצע שלושת ערוצי הצבע שלו.
...
הממוצע לאורך ציר הערוצים הופך שלושה ערכים לערך אחד בכל פיקסל. זו המחשה פשוטה לגווני אפור, ולא המרה המשקללת את רגישות העין לצבעים.
...
שינוי ערכי המערך או סידורם משנה את התמונה המוצגת. נדגים היפוך של ערכי הצבע, סיבוב התמונה והצגת הערוץ האדום בלבד.
...
`255 - pixels` הופכת כל ערך צבע. `rot90` עם `k=-1` מסובבת רבע סיבוב עם כיוון השעון. האינדקס האחרון `0` בוחר את הערוץ האדום; שאר הערוצים נשארים אפס. `zip` מצמידה כל אזור ציור למערך הבא ומפסיקה כשהרצף הקצר מסתיים.

...
<a href="21-PyPlot.md">→ הקודם</a>
<a class="toc-link" href="../index.md#book-toc">תוכן העניינים</a>
<a href="23-Pandas%20%D7%98%D7%91%D7%9C%D7%90%D7%95%D7%AA.md">הבא ←</a>
</nav>
...
תמונה דיגיטלית היא טבלה של פיקסלים. בתמונת גווני אפור כל פיקסל מתואר בערך בהירות אחד; בתמונת RGB הוא מתואר בשלושה ערכים — אדום, ירוק וכחול. לכן אפשר לטעון תמונה למערך ולפעול עליה באמצעות NumPy.

הרעיון הזה הוא הבסיס לזיהוי תמונות בלמידת מכונה. בחלק ג נבנה רשתות שמסווגות תמונות של בגדים ואותיות ורשת שמבדילה בין חתול ללא־חתול; בשני המקרים הרשת אינה מקבלת „תמונה”, אלא מערך של מספרים בדיוק כמו זה שניצור כאן, ולומדת למצוא בו דפוסים. בפרק הזה נטען תמונה למערך, נבדוק את צורתה, ונראה שפעולות חשבוניות פשוטות על המערך משנות את התמונה בדרכים מוכרות.
...
השאלה הראשונה שנשאל על כל תמונה היא: מה גודלה וכמה ערוצי צבע יש בה? התשובה מסתתרת בצורת המערך, בדיוק כפי שלמדנו בפרק על NumPy. צורת המערך מתארת את ממדי התמונה ואת מספר ערוצי הצבע שלה. נבדוק זאת באמצעות דוגמת תמונת הקמפוס במחברת 14. לאחר הורדת קובץ הליווי בנתיב המופיע במחברת, הקוד טוען אותו בעזרת `Image` מספריית Pillow, ספרייה לפתיחת קובצי תמונה; הקריאה `np.array(image)` ממירה את התמונה למערך NumPy, ו־`plt.imshow` של PyPlot מציגה מערך כתמונה:
...
זהו הגודל השמור לדוגמת הקמפוס: 500 שורות, 750 עמודות ושלושה ערוצים. במילים אחרות, זהו מערך תלת־ממדי: שני הצירים הראשונים הם גובה ורוחב התמונה בפיקסלים, והציר השלישי מחזיק לכל פיקסל את שלושת ערכי הצבע — אדום, ירוק וכחול. לתמונה אחרת יכולה להיות צורה אחרת. `uint8` שומר מספרים שלמים בתחום 0–255: `0` הוא היעדר מוחלט של הצבע ו־`255` הוא עוצמתו המלאה, כך שפיקסל `(255, 255, 255)` הוא לבן ו־`(0, 0, 0)` הוא שחור. `plt.axis("off")` מסתירה את הצירים והמספרים סביב התמונה.

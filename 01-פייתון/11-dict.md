...
**`in` במילון בודק מפתחות.** גישה למפתח חסר באמצעות סוגריים מרובעים גורמת ל־`KeyError`. מפתחות צריכים להיות ניתנים לגיבוב (`hashable`), למשל מספר או מחרוזת; רשימה אינה מתאימה. tuple מתאים אם כל איבריו מתאימים כמפתחות. הערכים יכולים להיות מכל טיפוס.
...
`for key in scores` עוברת על המפתחות. ב־`for name, score in scores.items()` כל זוג נפרק לשני משתנים, כפי שלמדנו בפרק tuple. סדר הסריקה הוא סדר הכנסת המפתחות למילון, ולא מיון שלהם. גם במילון **`copy()` היא העתקה שטחית**; למבנים פנימיים שצריכים העתקה נפרדת משתמשים ב־`deepcopy`.

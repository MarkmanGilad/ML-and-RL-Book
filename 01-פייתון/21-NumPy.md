...
<a href="20-%D7%A7%D7%91%D7%A6%D7%99%D7%9D.md">→ הקודם</a>
<a class="toc-link" href="../index.md#book-toc">תוכן העניינים</a>
<a href="22-PyPlot.md">הבא ←</a>
</nav>
...
**[פתיחת המחברת ב־Colab](https://colab.research.google.com/drive/1ROOlcw8QjYscDVM7ACz7iUCyenJIoGnl?usp=share_link) · [השיעור וההרצאה באתר הקורס](https://webprogramming.azurewebsites.net/Pages/Python/Numpy.aspx)**
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L32-L36] -->
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L38-L62] -->
...
גם כאן התוצאה היא מערך NumPy. [יצירת מערך מרצף בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.array.html).
...
ללא `dtype=int`, הפונקציות `zeros` ו־`ones` יוצרות כברירת מחדל מספרים מסוג `float`, למשל `0.` או `1.`. [מערך של אחדים בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.ones.html).
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L68-L91] -->

### ממדים וצורה

לפני שנמשיך לפעולות נוספות, נבין כיצד NumPy מתארת את המבנה של מערך. שני מושגים ילוו אותנו מכאן ועד סוף הספר: **מספר הממדים** ו**הצורה — shape**. כמעט כל שגיאה בעבודה עם מערכים, ובהמשך עם טנזורים בלמידת מכונה, קשורה לצורה שאינה מתאימה, ולכן כדאי להכיר את המושגים היטב.
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L103-L145] -->

<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L182-L211] -->
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L155-L178] -->
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L304-L333] -->
...
זהו אותו אוסף מספרים, אך בצורות שונות. ההבדל חשוב בהמשך, כאשר פעולה דורשת מערכים בצורה מסוימת. [אינדקסים וחיתוך בתיעוד NumPy](https://numpy.org/doc/stable/user/basics.indexing.html).
...
הכלל אחיד: פסיק מפריד בין צירים; נקודתיים מגדירות טווח בתוך ציר. [חיתוך רב־ממדי בתיעוד NumPy](https://numpy.org/doc/stable/user/basics.indexing.html).
...
השינוי ב־`part` משנה גם את `values`; `saved` נשארת נפרדת. זה שונה מחיתוך של list, שיוצר רשימה חיצונית חדשה. [תצוגות והעתקות בתיעוד NumPy](https://numpy.org/doc/stable/user/basics.copies.html).
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L223-L265] -->
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L278-L298] -->
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L339-L365] -->
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L377-L405] -->
...
<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L411-L438] -->
...
הפונקציה `function_values` משתמשת ב־`f` שהגדרנו קודם. כל איבר ברשימת התוצאות מתאים לאיבר באותו מיקום ברשימת הקלט. [המרת מערך לרשימה בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.ndarray.tolist.html).
...
כל תוצאה נשארת במיקום של הקלט המתאים: השורש של `4` הוא `2`, ולכן האיבר השלישי בתוצאה הוא `2.`. [שורש ריבועי בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.sqrt.html).
...
כאן התקבלו חמש נקודות במרווח `2`, ו־`10` אינו ביניהן. [יצירת סדרה באמצעות linspace בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.linspace.html).
...
כדי שהפלט יהיה קריא השתמשנו ב־`np.round(y, 4)`, שמחזירה את הערכים מעוגלים לארבע ספרות אחרי הנקודה. העיגול נעשה כאן לצורך ההדפסה; המערך `y` נשאר עם הערכים שחושבו. [עיגול בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.round.html).
...
הפונקציות `np.sin` ו־`np.cos` מחשבות סינוס וקוסינוס לכל זווית במערך. **הקלט שלהן נמדד ברדיאנים**, ולא במעלות. ברדיאנים סיבוב שלם הוא `2π`, וחצי סיבוב הוא `π`, המקביל ל־180 מעלות. הקבוע `np.pi` מספק את ערכו של π, בקירוב `3.14159`.
...
חישובי המחשב משתמשים בקירובים עשרוניים. לכן ערך שאמור להיות אפס, כמו סינוס של π, עשוי להיות מספר זעיר הקרוב לאפס. העיגול מסביר מדוע הוא מוצג כאן כ־`0.0`. [סינוס בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.sin.html), [קוסינוס בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.cos.html).
...
הפלט כולל את `e⁰`, את `e¹` ואת `e²`, באותו סדר כמו מערך הקלט. [הפונקציה המעריכית בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.exp.html).
...
קיבלנו בחזרה, עד כדי דיוק החישוב, את הערכים המקוריים שיצרנו עם `linspace`. השם `log` ב־NumPy מציין לוגריתם בבסיס `e`. [לוגריתם טבעי בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.log.html).
...
לכן חיבור ישיר של `matrix` עם `np.array([10, 20])` יגרום ל־`ValueError`. מספר השורות לבדו אינו קובע התאמה. Broadcasting אינו מוסיף איברים למערכי המקור; הוא מתאים את השימוש בנתונים במהלך החישוב. [כללי broadcasting בתיעוד NumPy](https://numpy.org/doc/stable/user/basics.broadcasting.html).
...
כדאי להיזהר מהקיצור „ציר השורות”: סיכום לאורך ציר השורות נותן תוצאה **לכל עמודה**. [סכימה וצירים בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.sum.html).
...
למשל, ממוצע התלמיד הראשון הוא `(10 + 20 + 30) / 3 = 20`. ממוצע המבחן הראשון הוא `(10 + 40) / 2 = 25`. [ממוצע בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.mean.html).
...
הפונקציות מחזירות את **הערכים**, ולא את האינדקסים שבהם הם נמצאים. [מינימום וצירים בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.min.html).
...
בביטוי `scores[passed]` נשמרים רק הציונים שעבורם המסכה מכילה `True`. זו בחירה לפי תנאי, ולא חיתוך לפי טווח מיקומים. [בחירה לפי תנאי במדריך NumPy](https://numpy.org/doc/stable/user/absolute_beginners.html).
...
אלו מיקומי הציונים `75` ו־`90`, ולא הציונים עצמם. [בחירת ערכים ומיקומים בתיעוד where](https://numpy.org/doc/stable/reference/generated/numpy.where.html).
...
כאן `-5` הוחלף ב־`0`, ו־`105` הוחלף ב־`100`. [הגבלת ערכים בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.clip.html).
...
הפלט הראשון הוא הכפל איבר מול איבר שכבר מכירים. הפלט השני שונה לגמרי. לדוגמה, האיבר הראשון בתוצאת כפל המטריצות הוא `1 * 10 + 2 * 30`, כלומר `70`: השורה הראשונה של `a` מול העמודה הראשונה של `b`. באופן כללי, כפל מטריצה בצורה `(m, n)` במטריצה בצורה `(n, k)` מחזיר מטריצה בצורה `(m, k)`. במחברת הקורס שתי המטריצות הן בצורה `(2, 3)`, ולכן משחלפים את השנייה לצורה `(3, 2)` לפני הכפל. [כפל מטריצות בתיעוד NumPy](https://numpy.org/doc/stable/reference/generated/numpy.matmul.html).

<!-- editorlm-source-ref: [sources/Python/converted/3.numpy/notebook.md#L411-L447] -->

...
תנאי מאפשר לתוכנית לבחור פעולה לפי הנתונים. למשל, אותו משחק יכול להציג ניצחון כשהניקוד גבוה ולהמשיך לשחק כשהוא נמוך. נכתוב את הבדיקה ואת ההוראות שיבוצעו בכל מצב.
...
`if` בודקת תנאי, `elif` בודקת תנאי נוסף אם הקודמים לא התקיימו, ו־`else` מטפלת במקרה שנותר. **בסוף שורת הכותרת כותבים נקודתיים (`:`).**
...
הביטוי `0 <= score < 10` בודק שני גבולות. השוואה לשוויון נכתבת `==`; השמה נכתבת `=`.

ביטויים בוליאניים משלבים בעזרת **המילים `and`, `or` ו־`not`**. הערכים **`True` ו־`False` מתחילים באות גדולה**.

...
<a href="../index.md">→ הקודם</a>
<a class="toc-link" href="../index.md#book-toc">תוכן העניינים</a>
<a href="02-%D7%A1%D7%91%D7%99%D7%91%D7%AA%20%D7%94%D7%A2%D7%91%D7%95%D7%93%D7%94.md">הבא ←</a>
</nav>

## א.1 — מהי פייתון?

פייתון היא שפת תכנות: באמצעותה כותבים הוראות שהמחשב מבצע. השפה מדגישה קוד פשוט וקריא, ובעזרתה נכתוב בקורס תוכניות שעובדות עם נתונים ובהמשך לומדות מהם.

למה דווקא פייתון? הספר הזה עוסק בלמידת מכונה ובבניית משחקים, ושני התחומים האלה נשענים על ספריות מוכנות — Pygame למשחקים, NumPy ו־PyTorch לחישובים וללמידה — שכולן כתובות לפייתון. לפני שנוכל ללמד מחשב לשחק, אנחנו צריכים שפה משותפת שבה נסביר לו מה לעשות. חלק א של הספר מלמד את השפה הזאת צעד אחר צעד: בפרק הזה נכיר מהי פייתון ומאין באה, בפרק הבא נקים סביבת עבודה, ומשם נעבור לפקודות, למשתנים, לתנאים וללולאות.

<div class="python-intro">
<img src="../assets/python-logo.png" alt="סמל פייתון — שני הנחשים בכחול ובצהוב" width="105">
<div><strong>Python</strong><br>שפה פשוטה לקריאה, עם עולם של אפשרויות.</div>
</div>
...
איך תוכנית שכתבנו הופכת לפעולה של המחשב? בשפות רבות יש שלב ביניים: את הקוד מעבירים תחילה בתוכנת **הידור (Compiler)** שמתרגמת אותו לשפת המכונה, ורק אז מריצים. בפייתון הדרך קצרה יותר.

**פייתון היא שפת סקריפט המופעלת באמצעות אינטרפרטר (מפרש)** — תוכנה שמבצעת את הוראות התוכנית. בעבודה הרגילה אין צורך לבצע שלב הידור נפרד לפני ההרצה: כותבים ומריצים. במימוש הנפוץ, CPython, נוצר מאחורי הקלעים קוד ביניים; לכן „אין הידור כלל” אינו תיאור מדויק. [מילון המונחים של Python](https://docs.python.org/3/glossary.html#term-bytecode).

השפה שואפת להיות פשוטה וקריאה, ולהתקרב ככל האפשר לצורת הביטוי של בני אדם. הדגש הוא על ניסוח ברור ומעט סימנים מיותרים. [על השפה ועקרונותיה](https://docs.python.org/3/faq/general.html#what-is-python).

<!-- editorlm-source-ref: [sources/Python/Python_Intro_Colab.pptx#L7-L11] -->

### קצת היסטוריה

פייתון פותחה בידי **גידו ואן רוסום (Guido van Rossum)** בהולנד. הוא החל לעבוד עליה בסוף **1989**, ופרסם אותה לראשונה בפברואר **1991**. השם נלקח מסדרת הקומדיה „מונטי פייתון”. כיום פיתוח השפה נמשך כפרויקט קוד פתוח, לצד פעילות קרן Python. [הסיפור באתר Python](https://docs.python.org/3/faq/general.html).

<div class="python-fact">
<strong>כ־23 מיליון מפתחים</strong><br>
זהו אומדן קהילת משתמשי פייתון בתחילת 2025 לפי SlashData — אומדן מחקרי, ולא ספירה מדויקת.
</div>

מקור: [מחקר קהילות המפתחים, SlashData, מאי 2025](https://www.slashdata.co/post/global-developer-population-trends-2025-how-many-developers-are-there).

### שפה מרכזית בעולם ה־AI

פייתון היא אחת השפות המרכזיות לבינה מלאכותית וללמידת מכונה. הקריאות שלה, הקהילה הגדולה והספריות המוכנות מאפשרות להתמקד בנתונים ובמודלים. בקורס נשתמש בה כדי ללמד מחשבים ללמוד ולפעול במשחקים. [Python ותחום ה־AI — JetBrains, 2025](https://blog.jetbrains.com/pycharm/2025/09/why-is-python-so-popular/).

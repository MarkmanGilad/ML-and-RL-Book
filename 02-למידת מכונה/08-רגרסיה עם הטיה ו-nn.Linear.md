...
## ב.8 רגרסיה עם הטיה ו־nn.Linear


הקו שלמדנו עבר בראשית, ולכן לא היה יכול לתת תחזית שונה מאפס כאשר הקלט אפס. נוסיף כעת הטיה ונלמד שני פרמטרים יחד. לאחר שנחשב אותם במפורש, נראה כיצד nn.Linear מנהלת עבורנו בדיוק אותו מודל.
...
המודל `ŷ = w*x + b` אינו חייב לעבור בראשית. המשקל w קובע את השיפוע, וההטיה b מאפשרת להזיז את הקו למעלה ולמטה. שני הפרמטרים נלמדים יחד.

מחברת ההמשך יוצרת 100 דוגמאות באמצעות scikit-learn:
...
x_array, y_array = make_regression(
    ...
    noise=20, bias=50, random_state=1
)
...
<figure>
<img src="../assets/sources/ML/converted/5.2_Linear_Regresion/assets/cell-7-output-2-2.png" alt="מאה דוגמאות עם רעש מתוך מחברת ההמשך." style="max-width:100%;height:auto;">
<!-- editorlm-source-ref: [sources/ML/converted/5.2_Linear_Regresion/notebook.md#L82] -->
<figcaption>מאה דוגמאות עם רעש מתוך מחברת ההמשך.</figcaption>
</figure>

ל־x ול־y יש צורה `[100, 1]`: כל שורה היא דוגמה. כך ההפרש מחושב בין תחזית לתשובה של אותה דוגמה.
...
<figure>
<img src="../assets/sources/ML/converted/5.2_Linear_Regresion/assets/cell-17-output-2-2.png" alt="ההפסד השמור לאורך 1,000 צעדי האימון של w ושל b." style="max-width:100%;height:auto;">
<!-- editorlm-source-ref: [sources/ML/converted/5.2_Linear_Regresion/notebook.md#L344] -->
<figcaption>ההפסד השמור לאורך 1,000 צעדי האימון של w ושל b.</figcaption>
</figure>

### ללמוד שני פרמטרים יחד

עם הטיה, ההפסד הוא `mean((wx+b−y)²)`. הנגזרת לפי w כוללת כפל בקלט; הנגזרת לפי b היא `2·mean(wx+b−y)`. ההטיה משפיעה באותה מידה על כל התחזיות, ואילו המשקל משנה אותן לפי x.

אם הקשר האמיתי הוא `y=2x+5`, שינוי w בלבד לעולם לא ייתן תחזית 5 כאשר x=0. זהו חוסר במודל, ולא בעיה במספר צעדי האימון. הוספת b מאפשרת להזיז את כל הקו. גם את b מעבירים לאופטימייזר; פרמטר שלא נכלל בו לא יתעדכן באמצעות `step`.

בנתונים הרועשים של מחברת ההמשך, ההפסד השמור מתקרב לכ־332.568 בשני המימושים. לא נצפה לאפס: הקו מתאר מגמה, בעוד שחלק מהפיזור נובע מהרעש שהוספנו. התאמת כל נקודה בכוח אינה מטרת הרגרסיה הזאת.

### אותה רגרסיה בעזרת nn.Linear

**nn.Linear(1, 1)** מגדירה שכבה עם קלט אחד ופלט אחד, ומנהלת את w ואת b. כברירת מחדל נכללת הטיה. **model.parameters()** מחזירה את הפרמטרים שהאופטימייזר צריך לעדכן.
...
הקריאה model(x) מפעילה את המודל. אין צורך לכתוב בעצמנו את w*x+b, אבל זה עדיין החישוב של השכבה.
...
with torch.no_grad():
    prediction = model(x)
    test = torch.tensor([[0.], [1.], [1.5]])
    answers = model(test)
order = x[:, 0].argsort()
plt.scatter(x[:, 0], y[:, 0])
plt.plot(x[order, 0], prediction[order, 0])
plt.show()
```
...
<figure>
<img src="../assets/sources/ML/converted/5.2_Linear_Regresion/assets/cell-29-output-2-2.png" alt="הקו שנלמד באמצעות nn.Linear, מתוך הפלט השמור." style="max-width:100%;height:auto;">
<!-- editorlm-source-ref: [sources/ML/converted/5.2_Linear_Regresion/notebook.md#L550] -->
<figcaption>הקו שנלמד באמצעות nn.Linear, מתוך הפלט השמור.</figcaption>
</figure>

### לקרוא את הפרמטרים שהמודל למד
...
בפלט השמור של מחברת ההמשך המשקל הוא בקירוב 82.4845 וההטיה 54.0540. לכן עבור x=0 התחזית היא 54.0540; עבור x=1 היא 136.5385; עבור x=1.5 היא 177.7808. חישוב ידני כזה מחבר בין אובייקט `Linear` לבין המשוואה המוכרת.

המשקל של `Linear(1,1)` נשמר בצורת `[1,1]`, וההטיה בצורת `[1]`. המספר הראשון מתאר מספר פלטים והשני מספר קלטים. אין צורך ליצור טנסור w נוסף: השכבה כבר מנהלת את הפרמטר הנלמד.

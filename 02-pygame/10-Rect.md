...
עד עכשיו מיקמנו תמונה באמצעות זוג מספרים. כשצריך ליישר אותה למרכז, לבדוק את הקצה הימני שלה או להזיז אותה בלי לחשב מחדש כל נקודה, נוח לשמור את מסגרת התמונה באובייקט אחד. **Rect** הוא מלבן ששומר מיקום ומידות ומאפשר לגשת אליהם בכמה דרכים.
...
המאפיינים מתארים אותו מלבן. למשל, שינוי `center` מזיז גם את הקצוות; הוא אינו משנה את המידות. שינוי `width` משנה את רוחב המלבן, אך **אינו משנה את גודל התמונה**. לשינוי התמונה משתמשים ב־`transform.scale()` ומעדכנים את המלבן בהתאם.
...
<figure>
<img src="../assets/pygame/rect.png" alt="מסגרת מלבנית, מרכז ופינות: כמה דרכים לתאר את אותו מיקום." width="640">
...
`move()` מחזירה מלבן חדש ואינה משנה את המקורי. `move_ip()` משנה את המלבן הקיים במקום. אין לכתוב `rect = rect.move_ip(...)`, משום שערך ההחזרה שלה הוא `None`.

### משתמשים במלבן להצבת התמונה

במקום זוג קואורדינטות, מעבירים ל־`blit()` את המלבן:
...
המלבן שומר קואורדינטות שלמות. בתנועה לפי זמן עדיף לשמור מיקום עשרוני במשתנה נפרד, כמו בפרק הקודם, ולהעתיק את המיקום המעוגל ל־`rect` לצורך הציור.

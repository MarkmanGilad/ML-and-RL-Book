...
<a href="08-%D7%9E%D7%A7%D7%9C%D7%93%D7%AA%20%D7%95%D7%A2%D7%9B%D7%91%D7%A8.md">→ הקודם</a>
...
כדי ליצור אשליה של תנועה מציירים אותו עצם במקומות מעט שונים, במהירות. בכל פריים מוחקים את הציור הישן, מעדכנים את המיקום ומציירים שוב. העצם נשאר אותו עצם; המשתנים שמציינים את מיקומו משתנים.

**המצגת:** [מבוא ל־Pygame](../../../sources/PyGame/2.PyGame.pptx)

### מיקום ומהירות בכל פריים

נשמור את מיקום המרכז במשתנים `x` ו־`y`. המשתנה `speed_x` מציין כמה פיקסלים להוסיף ל־`x` בכל סיבוב. ערך חיובי מזיז ימינה וערך שלילי שמאלה.
...
למעגל יש רדיוס, ולכן מרכזו צריך להישאר במרחק הרדיוס מהקצוות. כאשר מגיעים לקצה, מציבים אותו בגבול והופכים את סימן המהירות.
...
בקצב של 60 פריימים בשנייה, שלושה פיקסלים לפריים הם כ־180 פיקסלים בשנייה. אם הקצב יורד, אותה תוספת לפריים תיצור תנועה איטית יותר. אפשר להגדיר את המהירות בפיקסלים לשנייה ולחשב מרחק לפי הזמן שעבר:

**מרחק = מהירות × זמן.**

הערך שמחזירה `clock.tick()` נמדד באלפיות שנייה. נחלק ב־1000 לקבלת שניות ונקרא לתוצאה `dt`.
...
בדיקת גבולות החלון נשארת כמו קודם. התאמה לזמן משפרת את עקביות מהירות התנועה; היא אינה מוסיפה תמונות שהמחשב לא הספיק לצייר.

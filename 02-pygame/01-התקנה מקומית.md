...
נריץ תוכנית קצרה כדי לוודא ש־Python מצליחה לטעון את Pygame. השורה `import pygame` היא הדרך שבה תוכנית Python מבקשת להשתמש בספרייה חיצונית; אם הספרייה מותקנת ונגישה למפרש, השורה תעבור בשקט, ואם לא — נקבל שגיאה מיד. בקובץ `check_pygame.py` כותבים:
